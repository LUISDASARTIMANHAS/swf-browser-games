onClipEvent(enterFrame){
   function §\x04\x05§()
   {
      set("\x03",575 % 511 * 5);
      return eval("\x03");
   }
   var §\x01§ = -105 + "\x04\x05"();
   loop0:
   while(true)
   {
      if(eval("\x01") == 215)
      {
         set("\x01",eval("\x01") - 15);
         §§push(true);
         continue;
      }
      if(eval("\x01") == 365)
      {
         set("\x01",eval("\x01") + 496);
         §§push(true);
         continue;
      }
      if(eval("\x01") != 37)
      {
         if(eval("\x01") == 723)
         {
            set("\x01",eval("\x01") - 235);
            var §§pop() = function §\x04\x05§()
            {
               set("\x03",575 % 511 * 5);
               return eval("\x03");
            };
         }
         else if(eval("\x01") == 661)
         {
            set("\x01",eval("\x01") - 62);
            §§push(eval(function §\x04\x05§()
            {
               set("\x03",575 % 511 * 5);
               return eval("\x03");
            }));
         }
         else if(eval("\x01") == 200)
         {
            set("\x01",eval("\x01") - 163);
            if(function §\x04\x05§()
            {
               set("\x03",575 % 511 * 5);
               return eval("\x03");
            })
            {
               set("\x01",eval("\x01") + 511);
            }
         }
         else
         {
            if(eval("\x01") == 802)
            {
               set("\x01",eval("\x01") - 266);
               if(eval(§§constant(5))[§§constant(6)](eval("{invalid_utf8:205}{invalid_utf8:228}{invalid_utf8:128}{invalid_utf8:134}")["d{invalid_utf8:210}{invalid_utf8:223}"][§§constant(2)][§§constant(3)][§§constant(4)]))
               {
                  if(eval("{invalid_utf8:205}{invalid_utf8:228}{invalid_utf8:128}{invalid_utf8:134}")[§§constant(7)] == 1)
                  {
                     eval("{invalid_utf8:205}{invalid_utf8:228}{invalid_utf8:128}{invalid_utf8:134}")[§§constant(8)] = eval("{invalid_utf8:205}{invalid_utf8:228}{invalid_utf8:128}{invalid_utf8:134}")[§§constant(9)];
                     eval("{invalid_utf8:205}{invalid_utf8:228}{invalid_utf8:128}{invalid_utf8:134}")[§§constant(9)] = eval("{invalid_utf8:205}{invalid_utf8:228}{invalid_utf8:128}{invalid_utf8:134}")[§§constant(10)];
                     eval("{invalid_utf8:205}{invalid_utf8:228}{invalid_utf8:128}{invalid_utf8:134}")[§§constant(10)] = §§constant(11);
                     eval("{invalid_utf8:205}{invalid_utf8:228}{invalid_utf8:128}{invalid_utf8:134}")[§§constant(12)][§§constant(13)](2);
                     eval("{invalid_utf8:205}{invalid_utf8:228}{invalid_utf8:128}{invalid_utf8:134}")["d{invalid_utf8:210}{invalid_utf8:223}"][§§constant(2)][§§constant(3)][§§constant(15)][§§constant(2)][§§constant(3)][§§constant(16)](§§constant(14));
                  }
               }
               break;
            }
            if(eval("\x01") == 947)
            {
               set("\x01",eval("\x01") - 582);
            }
            else if(eval("\x01") == 653)
            {
               set("\x01",eval("\x01") + 228);
               if(function §\x04\x05§()
               {
                  set("\x03",575 % 511 * 5);
                  return eval("\x03");
               })
               {
                  set("\x01",eval("\x01") - 79);
               }
            }
            else if(eval("\x01") == 247)
            {
               set("\x01",eval("\x01") + 476);
               §§push("\x0f");
               §§push(1);
            }
            else if(eval("\x01") == 881)
            {
               set("\x01",eval("\x01") - 79);
            }
            else if(eval("\x01") == 488)
            {
               set("\x01",eval("\x01") + 173);
               §§push("\x0f");
            }
            else if(eval("\x01") == 861)
            {
               set("\x01",eval("\x01") - 783);
               if(function §\x04\x05§()
               {
                  set("\x03",575 % 511 * 5);
                  return eval("\x03");
               })
               {
                  set("\x01",eval("\x01") + 547);
               }
            }
            else if(eval("\x01") == 830)
            {
               set("\x01",eval("\x01") - 583);
            }
            else if(eval("\x01") == 548)
            {
               set("\x01",eval("\x01") - 183);
            }
            else if(eval("\x01") == 625)
            {
               set("\x01",eval("\x01") - 378);
            }
            else
            {
               if(eval("\x01") == 78)
               {
                  set("\x01",eval("\x01") + 547);
                  break;
               }
               if(eval("\x01") != 599)
               {
                  if(eval("\x01") == 536)
                  {
                     set("\x01",eval("\x01") - 536);
                     break;
                  }
                  break;
               }
               set("\x01",eval("\x01") + 54);
               §§push(!function §\x04\x05§()
               {
                  set("\x03",575 % 511 * 5);
                  return eval("\x03");
               });
            }
         }
         continue;
      }
      set("\x01",eval("\x01") + 511);
      while(true)
      {
         if(eval("\x01") != 964)
         {
            if(eval("\x01") == 270)
            {
               set("\x01",eval("\x01") + 718);
            }
            else if(eval("\x01") == 809)
            {
               set("\x01",eval("\x01") - 651);
               §§push(eval(function §\x04\x05§()
               {
                  set("\x03",575 % 511 * 5);
                  return eval("\x03");
               }));
            }
            else if(eval("\x01") == 988)
            {
               set("\x01",eval("\x01") - 203);
               §§push(true);
            }
            else
            {
               if(eval("\x01") == 382)
               {
                  set("\x01",eval("\x01") + 130);
                  §§push(§§pop() % function §\x04\x05§()
                  {
                     set("\x03",575 % 511 * 5);
                     return eval("\x03");
                  });
                  break loop0;
               }
               if(eval("\x01") == 158)
               {
                  set("\x01",eval("\x01") + 154);
                  §§push(!function §\x04\x05§()
                  {
                     set("\x03",575 % 511 * 5);
                     return eval("\x03");
                  });
               }
               else if(eval("\x01") == 312)
               {
                  set("\x01",eval("\x01") - 197);
                  if(function §\x04\x05§()
                  {
                     set("\x03",575 % 511 * 5);
                     return eval("\x03");
                  })
                  {
                     set("\x01",eval("\x01") + 378);
                  }
               }
               else
               {
                  if(eval("\x01") != 115)
                  {
                     if(eval("\x01") == 505)
                     {
                        set("\x01",eval("\x01") - 505);
                        break loop0;
                     }
                     break loop0;
                  }
                  set("\x01",eval("\x01") + 378);
               }
            }
            while(true)
            {
               if(eval("\x01") == 688)
               {
                  set("\x01",eval("\x01") + 43);
                  §§push(true);
               }
               else if(eval("\x01") == 606)
               {
                  set("\x01",eval("\x01") + 203);
                  §§push("\x0f");
               }
               else
               {
                  if(eval("\x01") == 493)
                  {
                     set("\x01",eval("\x01") + 12);
                     eval("{invalid_utf8:205}{invalid_utf8:228}{invalid_utf8:128}{invalid_utf8:134}")["d{invalid_utf8:210}{invalid_utf8:223}"] = 0;
                     eval("{invalid_utf8:205}{invalid_utf8:228}{invalid_utf8:128}{invalid_utf8:134}")[§§constant(2)][§§constant(3)](§§constant(2));
                     break loop0;
                  }
                  if(eval("\x01") == 512)
                  {
                     set("\x01",eval("\x01") - 224);
                  }
                  else if(eval("\x01") == 288)
                  {
                     set("\x01",eval("\x01") + 254);
                     §§push("\x0f");
                     §§push(1);
                  }
                  else if(eval("\x01") == 532)
                  {
                     set("\x01",eval("\x01") + 456);
                  }
                  else if(eval("\x01") == 731)
                  {
                     set("\x01",eval("\x01") + 233);
                     if(function §\x04\x05§()
                     {
                        set("\x03",575 % 511 * 5);
                        return eval("\x03");
                     })
                     {
                        set("\x01",eval("\x01") - 432);
                     }
                  }
                  else if(eval("\x01") == 785)
                  {
                     set("\x01",eval("\x01") - 403);
                     if(function §\x04\x05§()
                     {
                        set("\x03",575 % 511 * 5);
                        return eval("\x03");
                     })
                     {
                        set("\x01",eval("\x01") + 130);
                     }
                  }
                  else if(eval("\x01") == 542)
                  {
                     set("\x01",eval("\x01") + 64);
                     var §§pop() = function §\x04\x05§()
                     {
                        set("\x03",575 % 511 * 5);
                        return eval("\x03");
                     };
                  }
                  else
                  {
                     if(eval("\x01") != 623)
                     {
                        break;
                     }
                     set("\x01",eval("\x01") - 335);
                  }
               }
            }
            continue;
         }
         set("\x01",eval("\x01") - 432);
         loop3:
         while(true)
         {
            while(true)
            {
               if(eval("\x01") == 589)
               {
                  set("\x01",eval("\x01") + 403);
                  §§push(true);
               }
               else if(eval("\x01") == 719)
               {
                  set("\x01",eval("\x01") - 608);
               }
               else if(eval("\x01") == 332)
               {
                  set("\x01",eval("\x01") - 267);
                  §§push(true);
               }
               else if(eval("\x01") == 924)
               {
                  set("\x01",eval("\x01") - 280);
                  if(function §\x04\x05§()
                  {
                     set("\x03",575 % 511 * 5);
                     return eval("\x03");
                  })
                  {
                     set("\x01",eval("\x01") - 229);
                  }
               }
               else if(eval("\x01") == 992)
               {
                  set("\x01",eval("\x01") - 30);
                  if(function §\x04\x05§()
                  {
                     set("\x03",575 % 511 * 5);
                     return eval("\x03");
                  })
                  {
                     set("\x01",eval("\x01") - 655);
                  }
               }
               else if(eval("\x01") == 997)
               {
                  set("\x01",eval("\x01") - 514);
                  §§push("\x0f");
               }
               else if(eval("\x01") == 329)
               {
                  set("\x01",eval("\x01") + 3);
               }
               else
               {
                  if(eval("\x01") != 111)
                  {
                     if(eval("\x01") == 962)
                     {
                        set("\x01",eval("\x01") - 655);
                        loop13:
                        while(true)
                        {
                           set(§§pop(),function §\x04\x05§()
                           {
                              set("\x03",575 % 511 * 5);
                              return eval("\x03");
                           } + 400);
                           while(true)
                           {
                              if(eval("\x01") == 735)
                              {
                                 set("\x01",eval("\x01") - 449);
                                 §§push(true);
                                 continue;
                              }
                              if(eval("\x01") == 563)
                              {
                                 set("\x01",eval("\x01") + 132);
                                 §§push(true);
                                 continue;
                              }
                              if(eval("\x01") == 926)
                              {
                                 set("\x01",eval("\x01") - 13);
                                 continue;
                              }
                              if(eval("\x01") == 913)
                              {
                                 set("\x01",eval("\x01") + 65);
                                 §§push(true);
                                 continue;
                              }
                              if(eval("\x01") == 95)
                              {
                                 set("\x01",eval("\x01") + 94);
                                 §§push(!§§pop());
                                 continue;
                              }
                              if(eval("\x01") == 286)
                              {
                                 set("\x01",eval("\x01") + 265);
                                 if(§§pop())
                                 {
                                    set("\x01",eval("\x01") + 65);
                                 }
                                 continue;
                              }
                              if(eval("\x01") == 417)
                              {
                                 set("\x01",eval("\x01") + 146);
                                 continue;
                              }
                              if(eval("\x01") == 551)
                              {
                                 set("\x01",eval("\x01") + 65);
                                 addr424:
                                 if(!§§pop())
                                 {
                                    if(eval("\x01") != 976)
                                    {
                                       if(eval("\x01") == 618)
                                       {
                                          set("\x01",eval("\x01") + 361);
                                          addr403:
                                          §§push(true);
                                          if(eval("\x01") == 314)
                                          {
                                             set("\x01",eval("\x01") + 644);
                                             §§goto(addr403);
                                             §§push(true);
                                          }
                                          §§goto(addr424);
                                          §§push(eval("\x01") == 974);
                                       }
                                       if(eval("\x01") == 362)
                                       {
                                          set("\x01",eval("\x01") + 206);
                                          §§goto(addr403);
                                       }
                                       if(eval("\x01") == 160)
                                       {
                                          set("\x01",eval("\x01") - 96);
                                          §§goto(addr403);
                                       }
                                       if(eval("\x01") == 887)
                                       {
                                          set("\x01",eval("\x01") + 87);
                                          if(!§§pop())
                                          {
                                             addr507:
                                             §§goto(addr403);
                                          }
                                          set("\x01",eval("\x01") - 814);
                                          §§goto(addr507);
                                       }
                                       if(eval("\x01") == 597)
                                       {
                                          set("\x01",eval("\x01") - 547);
                                          var §§pop() = §§pop();
                                          §§goto(addr403);
                                       }
                                       if(eval("\x01") == 64)
                                       {
                                          set("\x01",eval("\x01") + 533);
                                          §§goto(addr403);
                                          §§push("\x0f");
                                          §§push(1);
                                       }
                                       if(eval("\x01") == 210)
                                       {
                                          set("\x01",eval("\x01") - 146);
                                          §§goto(addr403);
                                       }
                                       if(eval("\x01") == 841)
                                       {
                                          set("\x01",eval("\x01") - 230);
                                          §§goto(addr403);
                                       }
                                       if(eval("\x01") == 915)
                                       {
                                          set("\x01",eval("\x01") - 118);
                                          §§goto(addr403);
                                          §§push(eval(§§pop()));
                                       }
                                       if(eval("\x01") == 50)
                                       {
                                          set("\x01",eval("\x01") + 865);
                                          §§goto(addr403);
                                          §§push("\x0f");
                                       }
                                       if(eval("\x01") == 797)
                                       {
                                          set("\x01",eval("\x01") + 142);
                                          §§goto(addr403);
                                          §§push(!§§pop());
                                       }
                                       if(eval("\x01") == 611)
                                       {
                                          set("\x01",eval("\x01") + 276);
                                          §§goto(addr403);
                                          §§push(true);
                                       }
                                       if(eval("\x01") == 346)
                                       {
                                          set("\x01",eval("\x01") + 272);
                                          §§goto(addr403);
                                       }
                                       if(eval("\x01") == 979)
                                       {
                                          set("\x01",eval("\x01") - 3);
                                          if(!§§pop())
                                          {
                                             addr653:
                                             §§goto(addr403);
                                          }
                                          set("\x01",eval("\x01") - 43);
                                          §§goto(addr653);
                                       }
                                       if(eval("\x01") == 340)
                                       {
                                          set("\x01",eval("\x01") + 278);
                                          §§goto(addr403);
                                       }
                                       if(eval("\x01") == 939)
                                       {
                                          set("\x01",eval("\x01") - 577);
                                          if(!§§pop())
                                          {
                                             addr688:
                                             §§goto(addr403);
                                          }
                                          set("\x01",eval("\x01") + 206);
                                          §§goto(addr688);
                                       }
                                       if(eval("\x01") != 568)
                                       {
                                          if(eval("\x01") == 933)
                                          {
                                             set("\x01",eval("\x01") - 322);
                                             §§goto(addr403);
                                          }
                                          if(eval("\x01") != 255)
                                          {
                                             if(eval("\x01") == 958)
                                             {
                                                set("\x01",eval("\x01") - 703);
                                                if(!§§pop())
                                                {
                                                   addr749:
                                                   §§goto(addr403);
                                                }
                                                set("\x01",eval("\x01") + 91);
                                                §§goto(addr749);
                                             }
                                             if(eval("\x01") == 959)
                                             {
                                                set("\x01",eval("\x01") - 959);
                                                break loop0;
                                             }
                                             break loop0;
                                          }
                                          set("\x01",eval("\x01") + 91);
                                          set(§§pop(),§§pop() - §§pop());
                                          break loop0;
                                       }
                                       set("\x01",eval("\x01") + 391);
                                       if(eval(§§constant(5))[§§constant(6)](eval(",l,Q")["ٶ!{invalid_utf8:236}"][§§constant(2)][§§constant(3)][§§constant(4)]))
                                       {
                                          eval(",l,Q")["ٶ!{invalid_utf8:236}"][§§constant(2)][§§constant(3)][§§constant(7)][§§constant(8)][§§constant(9)][§§constant(10)]();
                                       }
                                       break loop0;
                                    }
                                    set("\x01",eval("\x01") - 43);
                                    break loop0;
                                 }
                                 set("\x01",eval("\x01") - 814);
                                 stop();
                                 §§push(String(§§pop()));
                                 break loop0;
                              }
                              if(eval("\x01") == 616)
                              {
                                 set("\x01",eval("\x01") + 297);
                                 continue;
                              }
                              if(eval("\x01") == 832)
                              {
                                 break loop13;
                              }
                              if(eval("\x01") == 978)
                              {
                                 set("\x01",eval("\x01") - 146);
                                 if(§§pop())
                                 {
                                    set("\x01",eval("\x01") - 415);
                                 }
                                 continue;
                              }
                              if(eval("\x01") == 545)
                              {
                                 set("\x01",eval("\x01") + 18);
                                 continue;
                              }
                              if(eval("\x01") == 695)
                              {
                                 set("\x01",eval("\x01") - 229);
                                 if(§§pop())
                                 {
                                    set("\x01",eval("\x01") + 171);
                                 }
                                 continue;
                              }
                              if(eval("\x01") == 813)
                              {
                                 set("\x01",eval("\x01") - 28);
                                 §§push("\x0f");
                                 §§push(1);
                                 continue;
                              }
                              if(eval("\x01") != 466)
                              {
                                 if(eval("\x01") == 637)
                                 {
                                    set("\x01",eval("\x01") + 176);
                                    continue;
                                 }
                                 if(eval("\x01") == 229)
                                 {
                                    set("\x01",eval("\x01") + 584);
                                    continue;
                                 }
                                 if(eval("\x01") == 99)
                                 {
                                    set("\x01",eval("\x01") - 4);
                                    §§push(eval(§§pop()));
                                    continue;
                                 }
                                 if(eval("\x01") == 785)
                                 {
                                    set("\x01",eval("\x01") + 127);
                                    var §§pop() = §§pop();
                                    continue;
                                 }
                                 if(eval("\x01") == 904)
                                 {
                                    set("\x01",eval("\x01") - 644);
                                    eval("{invalid_utf8:143}{invalid_utf8:159}{invalid_utf8:141}")["\\4("]["\x11\x0f{invalid_utf8:178}="] += eval("{invalid_utf8:205}{invalid_utf8:225}\x13/") * eval("{invalid_utf8:143}{invalid_utf8:159}{invalid_utf8:141}")["{invalid_utf8:147}{invalid_utf8:166} {invalid_utf8:159}"][§§constant(5)];
                                    break loop0;
                                 }
                                 if(eval("\x01") == 912)
                                 {
                                    set("\x01",eval("\x01") - 813);
                                    §§push("\x0f");
                                    continue;
                                 }
                                 if(eval("\x01") == 189)
                                 {
                                    set("\x01",eval("\x01") + 315);
                                    if(§§pop())
                                    {
                                       set("\x01",eval("\x01") + 400);
                                    }
                                    continue;
                                 }
                                 if(eval("\x01") != 504)
                                 {
                                    if(eval("\x01") == 260)
                                    {
                                       set("\x01",eval("\x01") - 260);
                                       break loop0;
                                    }
                                    break loop0;
                                 }
                                 continue loop13;
                              }
                              set("\x01",eval("\x01") + 171);
                              §§pop()["{invalid_utf8:235}d"].Ic[§§constant(16)][§§constant(17)][§§constant(18)]();
                           }
                        }
                        set("\x01",eval("\x01") - 415);
                        prevFrame();
                        stop();
                        §§push(§§pop() == (§§pop() == String(function §\x04\x05§()
                        {
                           set("\x03",575 % 511 * 5);
                           return eval("\x03");
                        })));
                        break loop0;
                     }
                     if(eval("\x01") != 568)
                     {
                        if(eval("\x01") == 353)
                        {
                           set("\x01",eval("\x01") - 242);
                           continue;
                        }
                        if(eval("\x01") == 222)
                        {
                           set("\x01",eval("\x01") + 775);
                           var §§pop() = function §\x04\x05§()
                           {
                              set("\x03",575 % 511 * 5);
                              return eval("\x03");
                           };
                           continue;
                        }
                        if(eval("\x01") == 980)
                        {
                           break loop3;
                        }
                        if(eval("\x01") == 65)
                        {
                           set("\x01",eval("\x01") + 786);
                           if(function §\x04\x05§()
                           {
                              set("\x03",575 % 511 * 5);
                              return eval("\x03");
                           })
                           {
                              set("\x01",eval("\x01") - 498);
                           }
                           continue;
                        }
                        if(eval("\x01") == 307)
                        {
                           set("\x01",eval("\x01") + 25);
                           continue;
                        }
                        if(eval("\x01") != 851)
                        {
                           if(eval("\x01") == 73)
                           {
                              set("\x01",eval("\x01") + 117);
                           }
                           else if(eval("\x01") == 483)
                           {
                              set("\x01",eval("\x01") + 354);
                              §§push(eval(function §\x04\x05§()
                              {
                                 set("\x03",575 % 511 * 5);
                                 return eval("\x03");
                              }));
                           }
                           else
                           {
                              if(eval("\x01") == 102)
                              {
                                 break;
                              }
                              if(eval("\x01") == 837)
                              {
                                 set("\x01",eval("\x01") + 87);
                                 §§push(!function §\x04\x05§()
                                 {
                                    set("\x03",575 % 511 * 5);
                                    return eval("\x03");
                                 });
                              }
                              else if(eval("\x01") == 644)
                              {
                                 set("\x01",eval("\x01") - 229);
                              }
                              else
                              {
                                 if(eval("\x01") != 190)
                                 {
                                    if(eval("\x01") == 415)
                                    {
                                       set("\x01",eval("\x01") - 340);
                                       _root.game.inner += inner2 * _root.shark.hit2;
                                       break loop0;
                                    }
                                    if(eval("\x01") == 75)
                                    {
                                       set("\x01",eval("\x01") - 75);
                                       break loop0;
                                    }
                                    break loop0;
                                 }
                                 set("\x01",eval("\x01") + 32);
                                 §§push("\x0f");
                                 §§push(1);
                              }
                           }
                           continue;
                        }
                     }
                     set("\x01",eval("\x01") + 412);
                     if(function §\x04\x05§()
                     {
                        set("\x03",575 % 511 * 5);
                        return eval("\x03");
                     })
                     {
                        set("\x01",eval("\x01") - 907);
                     }
                     continue;
                     set("\x01",eval("\x01") - 498);
                     loop5:
                     while(true)
                     {
                        set(§§pop(),§§pop() + function §\x04\x05§()
                        {
                           set("\x03",575 % 511 * 5);
                           return eval("\x03");
                        });
                        §§push(true);
                        while(eval("\x01") != 6)
                        {
                           if(eval("\x01") == 867)
                           {
                              break loop5;
                           }
                           if(eval("\x01") == 166)
                           {
                              set("\x01",eval("\x01") + 531);
                              if(§§pop())
                              {
                                 set("\x01",eval("\x01") + 239);
                              }
                              continue;
                           }
                           if(eval("\x01") == 959)
                           {
                              set("\x01",eval("\x01") - 92);
                              if(§§pop())
                              {
                                 set("\x01",eval("\x01") - 464);
                              }
                              continue;
                           }
                           if(eval("\x01") == 687)
                           {
                              set("\x01",eval("\x01") - 610);
                              §§push(eval(§§pop()));
                              continue;
                           }
                           if(eval("\x01") == 232)
                           {
                              set("\x01",eval("\x01") + 41);
                              if(§§pop())
                              {
                                 set("\x01",eval("\x01") + 20);
                              }
                              continue;
                           }
                           if(eval("\x01") == 117)
                           {
                              set("\x01",eval("\x01") + 305);
                              §§push("\x0f");
                              §§push(1);
                              continue;
                           }
                           if(eval("\x01") == 273)
                           {
                              set("\x01",eval("\x01") + 20);
                              continue;
                           }
                           if(eval("\x01") == 580)
                           {
                              set("\x01",eval("\x01") - 463);
                              continue;
                           }
                           if(eval("\x01") == 403)
                           {
                              set("\x01",eval("\x01") + 252);
                              continue;
                           }
                           if(eval("\x01") == 697)
                           {
                              set("\x01",eval("\x01") + 239);
                              break loop0;
                           }
                           if(eval("\x01") == 422)
                           {
                              set("\x01",eval("\x01") + 300);
                              var §§pop() = §§pop();
                              continue;
                           }
                           if(eval("\x01") == 79)
                           {
                              set("\x01",eval("\x01") + 880);
                              §§push(true);
                              continue;
                           }
                           if(eval("\x01") == 77)
                           {
                              set("\x01",eval("\x01") + 155);
                              §§push(!§§pop());
                              continue;
                           }
                           if(eval("\x01") == 606)
                           {
                              set("\x01",eval("\x01") - 250);
                              if(§§pop())
                              {
                                 set("\x01",eval("\x01") + 154);
                              }
                              continue;
                           }
                           if(eval("\x01") == 936)
                           {
                              set("\x01",eval("\x01") - 857);
                              continue;
                           }
                           if(eval("\x01") == 655)
                           {
                              set("\x01",eval("\x01") - 49);
                              §§push(true);
                              continue;
                           }
                           if(eval("\x01") == 722)
                           {
                              set("\x01",eval("\x01") - 35);
                              §§push("\x0f");
                              continue;
                           }
                           if(eval("\x01") == 510)
                           {
                              set("\x01",eval("\x01") - 393);
                              continue;
                           }
                           if(eval("\x01") == 293)
                           {
                              set("\x01",eval("\x01") - 252);
                              eval("{invalid_utf8:254}[{invalid_utf8:146}D")["{invalid_utf8:209}{invalid_utf8:253}\x0b"][")ߒJ"] += eval("{invalid_utf8:162}Z\x06") * eval("{invalid_utf8:254}[{invalid_utf8:146}D")[§§constant(4)][§§constant(5)];
                              break loop0;
                           }
                           if(eval("\x01") == 41)
                           {
                              set("\x01",eval("\x01") - 41);
                              break loop0;
                           }
                           if(eval("\x01") == 962)
                           {
                              set("\x01",eval("\x01") - 883);
                              continue;
                           }
                           if(eval("\x01") == 504)
                           {
                              set("\x01",eval("\x01") + 151);
                              continue;
                           }
                           if(eval("\x01") != 356)
                           {
                              break loop0;
                           }
                           set("\x01",eval("\x01") + 154);
                           loop7:
                           while(true)
                           {
                              set(§§pop(),eval("\x01") - 380);
                              loop8:
                              while(true)
                              {
                                 while(true)
                                 {
                                    if(eval("\x01") == 957)
                                    {
                                       set("\x01",eval("\x01") - 275);
                                       §§push(true);
                                    }
                                    else if(eval("\x01") == 682)
                                    {
                                       set("\x01",eval("\x01") - 627);
                                       if(§§pop())
                                       {
                                          set("\x01",eval("\x01") + 103);
                                       }
                                    }
                                    else if(eval("\x01") == 672)
                                    {
                                       set("\x01",eval("\x01") - 84);
                                       §§push(eval(§§pop()));
                                    }
                                    else
                                    {
                                       if(eval("\x01") == 55)
                                       {
                                          break loop8;
                                       }
                                       if(eval("\x01") == 158)
                                       {
                                          set("\x01",eval("\x01") - 109);
                                       }
                                       else if(eval("\x01") == 704)
                                       {
                                          set("\x01",eval("\x01") - 655);
                                       }
                                       else if(eval("\x01") == 49)
                                       {
                                          set("\x01",eval("\x01") + 267);
                                          §§push("\x0f");
                                          §§push(1);
                                       }
                                       else
                                       {
                                          if(eval("\x01") == 583)
                                          {
                                             set("\x01",eval("\x01") - 466);
                                             eval("i(")["{invalid_utf8:239}{invalid_utf8:157}%"] = 0;
                                             eval("i(")[§§constant(2)][§§constant(3)](0);
                                             eval("i(")[§§constant(4)] = 0;
                                             eval("i(")[§§constant(5)][§§constant(6)](§§constant(5));
                                             break loop0;
                                          }
                                          if(eval("\x01") == 316)
                                          {
                                             set("\x01",eval("\x01") - 311);
                                             var §§pop() = §§pop();
                                          }
                                          else if(eval("\x01") == 5)
                                          {
                                             set("\x01",eval("\x01") + 667);
                                             §§push("\x0f");
                                          }
                                          else
                                          {
                                             if(eval("\x01") == 673)
                                             {
                                                break;
                                             }
                                             if(eval("\x01") == 963)
                                             {
                                                set("\x01",eval("\x01") - 380);
                                             }
                                             else
                                             {
                                                if(eval("\x01") != 588)
                                                {
                                                   if(eval("\x01") == 117)
                                                   {
                                                      set("\x01",eval("\x01") - 117);
                                                      break loop0;
                                                   }
                                                   break loop0;
                                                }
                                                set("\x01",eval("\x01") + 85);
                                                §§push(!§§pop());
                                             }
                                          }
                                       }
                                    }
                                 }
                                 set("\x01",eval("\x01") + 290);
                                 if(!§§pop())
                                 {
                                    continue;
                                 }
                                 continue loop7;
                              }
                              set("\x01",eval("\x01") + 103);
                              break loop0;
                           }
                        }
                     }
                     set("\x01",eval("\x01") - 464);
                     loop10:
                     while(true)
                     {
                        set(§§pop(),eval(function §\x04\x05§()
                        {
                           set("\x03",575 % 511 * 5);
                           return eval("\x03");
                        }) + 45);
                        loop11:
                        while(true)
                        {
                           while(true)
                           {
                              if(eval("\x01") == 759)
                              {
                                 set("\x01",eval("\x01") - 584);
                                 §§push(true);
                                 continue;
                              }
                              if(eval("\x01") == 175)
                              {
                                 set("\x01",eval("\x01") + 634);
                                 if(§§pop())
                                 {
                                    set("\x01",eval("\x01") + 74);
                                 }
                                 continue;
                              }
                              if(eval("\x01") == 226)
                              {
                                 set("\x01",eval("\x01") - 4);
                                 §§push(!§§pop());
                                 continue;
                              }
                              if(eval("\x01") == 506)
                              {
                                 set("\x01",eval("\x01") + 81);
                                 continue;
                              }
                              if(eval("\x01") == 222)
                              {
                                 set("\x01",eval("\x01") + 284);
                                 if(§§pop())
                                 {
                                    set("\x01",eval("\x01") + 81);
                                 }
                                 continue;
                              }
                              if(eval("\x01") != 587)
                              {
                                 if(eval("\x01") == 495)
                                 {
                                    set("\x01",eval("\x01") + 305);
                                    §§push("\x0f");
                                 }
                                 else
                                 {
                                    if(eval("\x01") == 809)
                                    {
                                       break loop11;
                                    }
                                    if(eval("\x01") == 948)
                                    {
                                       set("\x01",eval("\x01") + 45);
                                       break loop0;
                                    }
                                    if(eval("\x01") == 883)
                                    {
                                       set("\x01",eval("\x01") - 458);
                                    }
                                    else if(eval("\x01") == 536)
                                    {
                                       set("\x01",eval("\x01") - 111);
                                    }
                                    else if(eval("\x01") == 425)
                                    {
                                       set("\x01",eval("\x01") - 232);
                                       §§push(true);
                                    }
                                    else
                                    {
                                       if(eval("\x01") == 193)
                                       {
                                          break;
                                       }
                                       if(eval("\x01") == 173)
                                       {
                                          set("\x01",eval("\x01") + 322);
                                          var §§pop() = §§pop();
                                       }
                                       else if(eval("\x01") == 993)
                                       {
                                          set("\x01",eval("\x01") - 33);
                                       }
                                       else if(eval("\x01") == 350)
                                       {
                                          set("\x01",eval("\x01") + 610);
                                       }
                                       else if(eval("\x01") == 960)
                                       {
                                          set("\x01",eval("\x01") - 787);
                                          §§push("\x0f");
                                          §§push(1);
                                       }
                                       else
                                       {
                                          if(eval("\x01") != 800)
                                          {
                                             if(eval("\x01") == 285)
                                             {
                                                set("\x01",eval("\x01") - 285);
                                                break loop0;
                                             }
                                             break loop0;
                                          }
                                          set("\x01",eval("\x01") - 574);
                                          §§push(eval(§§pop()));
                                       }
                                    }
                                 }
                                 continue;
                              }
                              set("\x01",eval("\x01") - 302);
                              if(eval(§§constant(6))[§§constant(7)](eval("s{invalid_utf8:180}")["2{invalid_utf8:133}U"]["p{invalid_utf8:190}{invalid_utf8:158}"]["\x06\n{invalid_utf8:145}"][§§constant(4)]["p{invalid_utf8:190}{invalid_utf8:158}"]["\x06\n{invalid_utf8:145}"][§§constant(5)]))
                              {
                                 eval("s{invalid_utf8:180}")[§§constant(8)] = 3;
                                 eval("s{invalid_utf8:180}")[§§constant(9)] = eval("s{invalid_utf8:180}")[§§constant(10)];
                                 eval("s{invalid_utf8:180}")[§§constant(10)] = eval("s{invalid_utf8:180}")[§§constant(11)];
                                 eval("s{invalid_utf8:180}")[§§constant(11)] = §§constant(12);
                                 eval("s{invalid_utf8:180}")[§§constant(13)][§§constant(14)](2);
                                 §§goto(addr68);
                                 §§push(§§constant(15));
                                 §§push(1);
                                 §§push(eval("s{invalid_utf8:180}")["2{invalid_utf8:133}U"]);
                              }
                           }
                           set("\x01",eval("\x01") + 755);
                           if(!§§pop())
                           {
                              continue;
                           }
                           continue loop10;
                        }
                        set("\x01",eval("\x01") + 74);
                        break loop0;
                     }
                     break loop0;
                  }
                  set("\x01",eval("\x01") + 457);
                  §§push(true);
               }
            }
            set("\x01",eval("\x01") + 88);
         }
         set("\x01",eval("\x01") - 907);
         §§push(function §\x04\x05§()
         {
            set("\x03",575 % 511 * 5);
            return eval("\x03");
         } + getTimer());
         loop15:
         while(true)
         {
            set(§§pop(),§§pop() + 242);
            loop16:
            while(true)
            {
               while(true)
               {
                  if(eval("\x01") == 251)
                  {
                     set("\x01",eval("\x01") - 9);
                     §§push(true);
                  }
                  else if(eval("\x01") == 248)
                  {
                     set("\x01",eval("\x01") + 509);
                  }
                  else
                  {
                     if(eval("\x01") == 550)
                     {
                        set("\x01",eval("\x01") + 242);
                        break loop0;
                     }
                     if(eval("\x01") == 173)
                     {
                        set("\x01",eval("\x01") + 608);
                        if(§§pop())
                        {
                           set("\x01",eval("\x01") - 230);
                        }
                     }
                     else
                     {
                        if(eval("\x01") == 551)
                        {
                           set("\x01",eval("\x01") - 40);
                           _root.board.score += z * _root.kombi.combo;
                           break loop0;
                        }
                        if(eval("\x01") == 519)
                        {
                           set("\x01",eval("\x01") - 259);
                           §§push("\x0f");
                        }
                        else if(eval("\x01") == 759)
                        {
                           set("\x01",eval("\x01") - 586);
                           §§push(!§§pop());
                        }
                        else if(eval("\x01") == 781)
                        {
                           set("\x01",eval("\x01") - 230);
                        }
                        else if(eval("\x01") == 701)
                        {
                           set("\x01",eval("\x01") - 182);
                           var §§pop() = §§pop();
                        }
                        else if(eval("\x01") == 877)
                        {
                           set("\x01",eval("\x01") - 120);
                        }
                        else if(eval("\x01") == 242)
                        {
                           set("\x01",eval("\x01") + 618);
                           if(§§pop())
                           {
                              set("\x01",eval("\x01") - 612);
                           }
                        }
                        else if(eval("\x01") == 260)
                        {
                           set("\x01",eval("\x01") + 499);
                           §§push(eval(§§pop()));
                        }
                        else
                        {
                           if(eval("\x01") == 860)
                           {
                              break loop16;
                           }
                           if(eval("\x01") == 893)
                           {
                              set("\x01",eval("\x01") - 131);
                              break loop0;
                           }
                           if(eval("\x01") == 752)
                           {
                              set("\x01",eval("\x01") - 751);
                              §§push(true);
                           }
                           else if(eval("\x01") == 757)
                           {
                              set("\x01",eval("\x01") - 227);
                              §§push(true);
                           }
                           else if(eval("\x01") == 530)
                           {
                              set("\x01",eval("\x01") + 363);
                              if(§§pop())
                              {
                                 set("\x01",eval("\x01") - 131);
                              }
                           }
                           else if(eval("\x01") == 762)
                           {
                              set("\x01",eval("\x01") - 10);
                           }
                           else
                           {
                              if(eval("\x01") == 1)
                              {
                                 break;
                              }
                              if(eval("\x01") == 569)
                              {
                                 set("\x01",eval("\x01") + 132);
                                 §§push("\x0f");
                                 §§push(1);
                              }
                              else
                              {
                                 if(eval("\x01") == 511)
                                 {
                                    set("\x01",eval("\x01") - 511);
                                    break loop0;
                                 }
                                 if(eval("\x01") == 792)
                                 {
                                    set("\x01",eval("\x01") - 223);
                                 }
                                 else if(eval("\x01") == 736)
                                 {
                                    set("\x01",eval("\x01") - 167);
                                 }
                                 else
                                 {
                                    if(eval("\x01") != 356)
                                    {
                                       break loop0;
                                    }
                                    set("\x01",eval("\x01") + 396);
                                 }
                              }
                           }
                        }
                     }
                  }
               }
               set("\x01",eval("\x01") + 549);
               if(!§§pop())
               {
                  continue;
               }
               continue loop15;
            }
            set("\x01",eval("\x01") - 612);
            break loop0;
         }
      }
   }
}
