function §\x04\x05§()
{
   set("\x03",484 % 511 * 5);
   return eval("\x03");
}
var §\x01§ = -1941 + "\x04\x05"();
loop0:
while(true)
{
   if(eval("\x01") == 479)
   {
      set("\x01",eval("\x01") - 94);
      §§push(true);
   }
   else if(eval("\x01") == 415)
   {
      set("\x01",eval("\x01") - 290);
   }
   else if(eval("\x01") == 253)
   {
      set("\x01",eval("\x01") + 405);
      §§push("\x0f");
   }
   else if(eval("\x01") == 22)
   {
      set("\x01",eval("\x01") + 478);
      if(function §\x04\x05§()
      {
         set("\x03",484 % 511 * 5);
         return eval("\x03");
      })
      {
         set("\x01",eval("\x01") - 170);
      }
   }
   else
   {
      if(eval("\x01") == 330)
      {
         set("\x01",eval("\x01") + 3);
         eval("{invalid_utf8:131}{invalid_utf8:128}")["{invalid_utf8:173}\x11{invalid_utf8:204}V"]["P{invalid_utf8:182}\x1a"] += eval("{invalid_utf8:128}]{invalid_utf8:182}") * eval("{invalid_utf8:131}{invalid_utf8:128}")[§§constant(4)][§§constant(5)];
         break;
      }
      if(eval("\x01") == 943)
      {
         set("\x01",eval("\x01") - 690);
         var §§pop() = function §\x04\x05§()
         {
            set("\x03",484 % 511 * 5);
            return eval("\x03");
         };
      }
      else if(eval("\x01") == 125)
      {
         set("\x01",eval("\x01") + 818);
         §§push("\x0f");
         §§push(1);
      }
      else if(eval("\x01") == 658)
      {
         set("\x01",eval("\x01") + 298);
         §§push(eval(function §\x04\x05§()
         {
            set("\x03",484 % 511 * 5);
            return eval("\x03");
         }));
      }
      else if(eval("\x01") == 385)
      {
         set("\x01",eval("\x01") + 288);
         if(function §\x04\x05§()
         {
            set("\x03",484 % 511 * 5);
            return eval("\x03");
         })
         {
            set("\x01",eval("\x01") + 53);
         }
      }
      else if(eval("\x01") == 500)
      {
         set("\x01",eval("\x01") - 170);
      }
      else if(eval("\x01") == 956)
      {
         set("\x01",eval("\x01") - 934);
         §§push(!function §\x04\x05§()
         {
            set("\x03",484 % 511 * 5);
            return eval("\x03");
         });
      }
      else
      {
         if(eval("\x01") == 673)
         {
            set("\x01",eval("\x01") + 53);
            prevFrame();
            loop1:
            while(true)
            {
               while(eval("\x01") != 901)
               {
                  if(eval("\x01") == 924)
                  {
                     set("\x01",eval("\x01") - 188);
                     continue;
                  }
                  if(eval("\x01") == 637)
                  {
                     set("\x01",eval("\x01") + 287);
                     break loop0;
                  }
                  if(eval("\x01") == 694)
                  {
                     set("\x01",eval("\x01") - 604);
                     var §§pop() = function §\x04\x05§()
                     {
                        set("\x03",484 % 511 * 5);
                        return eval("\x03");
                     };
                     continue;
                  }
                  if(eval("\x01") == 817)
                  {
                     set("\x01",eval("\x01") - 513);
                     if(function §\x04\x05§()
                     {
                        set("\x03",484 % 511 * 5);
                        return eval("\x03");
                     })
                     {
                        set("\x01",eval("\x01") + 252);
                     }
                     continue;
                  }
                  if(eval("\x01") == 979)
                  {
                     set("\x01",eval("\x01") - 572);
                     §§push(eval(function §\x04\x05§()
                     {
                        set("\x03",484 % 511 * 5);
                        return eval("\x03");
                     }));
                     continue;
                  }
                  if(eval("\x01") == 757)
                  {
                     set("\x01",eval("\x01") - 555);
                     continue;
                  }
                  if(eval("\x01") == 90)
                  {
                     set("\x01",eval("\x01") + 889);
                     §§push("\x0f");
                     continue;
                  }
                  if(eval("\x01") == 346)
                  {
                     set("\x01",eval("\x01") + 291);
                     if(function §\x04\x05§()
                     {
                        set("\x03",484 % 511 * 5);
                        return eval("\x03");
                     })
                     {
                        set("\x01",eval("\x01") + 287);
                     }
                     continue;
                  }
                  if(eval("\x01") == 284)
                  {
                     set("\x01",eval("\x01") + 452);
                     continue;
                  }
                  if(eval("\x01") == 407)
                  {
                     set("\x01",eval("\x01") + 410);
                     §§push(!function §\x04\x05§()
                     {
                        set("\x03",484 % 511 * 5);
                        return eval("\x03");
                     });
                     continue;
                  }
                  if(eval("\x01") == 202)
                  {
                     set("\x01",eval("\x01") + 492);
                     §§push("\x0f");
                     §§push(1);
                     continue;
                  }
                  if(eval("\x01") == 304)
                  {
                     set("\x01",eval("\x01") + 252);
                     continue;
                  }
                  if(eval("\x01") == 764)
                  {
                     set("\x01",eval("\x01") - 562);
                     continue;
                  }
                  if(eval("\x01") == 689)
                  {
                     set("\x01",eval("\x01") - 22);
                     continue;
                  }
                  if(eval("\x01") == 556)
                  {
                     set("\x01",eval("\x01") - 222);
                     _root.board.score += z * _root.kombi.combo;
                     break loop0;
                  }
                  if(eval("\x01") == 334)
                  {
                     break loop1;
                  }
                  if(eval("\x01") == 736)
                  {
                     set("\x01",eval("\x01") - 72);
                     §§push(true);
                     continue;
                  }
                  if(eval("\x01") != 516)
                  {
                     if(eval("\x01") == 501)
                     {
                        set("\x01",eval("\x01") + 15);
                        if(function §\x04\x05§()
                        {
                           set("\x03",484 % 511 * 5);
                           return eval("\x03");
                        })
                        {
                           set("\x01",eval("\x01") + 248);
                        }
                     }
                     else if(eval("\x01") == 664)
                     {
                        set("\x01",eval("\x01") + 244);
                        if(function §\x04\x05§()
                        {
                           set("\x03",484 % 511 * 5);
                           return eval("\x03");
                        })
                        {
                           set("\x01",eval("\x01") - 219);
                        }
                     }
                     else
                     {
                        if(eval("\x01") == 908)
                        {
                           set("\x01",eval("\x01") - 219);
                           break loop0;
                        }
                        if(eval("\x01") == 667)
                        {
                           set("\x01",eval("\x01") - 166);
                           §§push(true);
                        }
                        else
                        {
                           if(eval("\x01") != 497)
                           {
                              break loop0;
                           }
                           set("\x01",eval("\x01") + 170);
                        }
                     }
                     continue;
                  }
                  set("\x01",eval("\x01") + 248);
                  loop3:
                  while(true)
                  {
                     §§push("\x0f");
                     while(true)
                     {
                        if(eval("\x01") == 715)
                        {
                           set("\x01",eval("\x01") + 189);
                           §§push(true);
                        }
                        else if(eval("\x01") == 639)
                        {
                           set("\x01",eval("\x01") - 276);
                           if(function §\x04\x05§()
                           {
                              set("\x03",484 % 511 * 5);
                              return eval("\x03");
                           })
                           {
                              set("\x01",eval("\x01") + 158);
                           }
                        }
                        else if(eval("\x01") == 441)
                        {
                           set("\x01",eval("\x01") - 333);
                           if(function §\x04\x05§()
                           {
                              set("\x03",484 % 511 * 5);
                              return eval("\x03");
                           })
                           {
                              set("\x01",eval("\x01") + 285);
                           }
                        }
                        else if(eval("\x01") == 904)
                        {
                           set("\x01",eval("\x01") - 31);
                           if(function §\x04\x05§()
                           {
                              set("\x03",484 % 511 * 5);
                              return eval("\x03");
                           })
                           {
                              set("\x01",eval("\x01") - 479);
                           }
                        }
                        else
                        {
                           if(eval("\x01") == 239)
                           {
                              break;
                           }
                           if(eval("\x01") == 393)
                           {
                              set("\x01",eval("\x01") - 156);
                           }
                           else if(eval("\x01") == 507)
                           {
                              set("\x01",eval("\x01") - 268);
                              var §§pop() = function §\x04\x05§()
                              {
                                 set("\x03",484 % 511 * 5);
                                 return eval("\x03");
                              };
                           }
                           else if(eval("\x01") == 237)
                           {
                              set("\x01",eval("\x01") + 270);
                              §§push("\x0f");
                              §§push(1);
                           }
                           else
                           {
                              if(eval("\x01") == 108)
                              {
                                 set("\x01",eval("\x01") + 285);
                                 return §§pop() and §§pop() < §§pop() - function §\x04\x05§()
                                 {
                                    set("\x03",484 % 511 * 5);
                                    return eval("\x03");
                                 };
                              }
                              if(eval("\x01") == 873)
                              {
                                 break loop3;
                              }
                              if(eval("\x01") == 432)
                              {
                                 set("\x01",eval("\x01") - 138);
                                 §§push(eval(function §\x04\x05§()
                                 {
                                    set("\x03",484 % 511 * 5);
                                    return eval("\x03");
                                 }));
                              }
                              else if(eval("\x01") == 52)
                              {
                                 set("\x01",eval("\x01") + 185);
                              }
                              else
                              {
                                 if(eval("\x01") == 521)
                                 {
                                    set("\x01",eval("\x01") + 314);
                                    _root.board.score += z * _root.kombi.combo;
                                    break loop0;
                                 }
                                 if(eval("\x01") == 363)
                                 {
                                    set("\x01",eval("\x01") + 158);
                                 }
                                 else if(eval("\x01") == 394)
                                 {
                                    set("\x01",eval("\x01") + 29);
                                 }
                                 else if(eval("\x01") == 423)
                                 {
                                    set("\x01",eval("\x01") + 18);
                                    §§push(true);
                                 }
                                 else if(eval("\x01") == 381)
                                 {
                                    set("\x01",eval("\x01") + 42);
                                 }
                                 else
                                 {
                                    if(eval("\x01") != 294)
                                    {
                                       if(eval("\x01") == 835)
                                       {
                                          set("\x01",eval("\x01") - 835);
                                          break loop0;
                                       }
                                       break loop0;
                                    }
                                    set("\x01",eval("\x01") + 345);
                                    §§push(!function §\x04\x05§()
                                    {
                                       set("\x03",484 % 511 * 5);
                                       return eval("\x03");
                                    });
                                 }
                              }
                           }
                        }
                     }
                     set("\x01",eval("\x01") + 193);
                  }
                  set("\x01",eval("\x01") - 479);
                  while(true)
                  {
                     if(function §\x04\x05§()
                     {
                        set("\x03",484 % 511 * 5);
                        return eval("\x03");
                     })
                     {
                        set("\x01",eval("\x01") + 51);
                     }
                     else if(eval("\x01") == 16)
                     {
                        set("\x01",eval("\x01") + 799);
                        §§push("\x0f");
                     }
                     else if(eval("\x01") == 513)
                     {
                        set("\x01",eval("\x01") - 281);
                     }
                     else if(eval("\x01") == 545)
                     {
                        set("\x01",eval("\x01") - 529);
                        var §§pop() = §§pop();
                     }
                     else
                     {
                        if(eval("\x01") == 728)
                        {
                           set("\x01",eval("\x01") - 694);
                           eval("a{invalid_utf8:178}{invalid_utf8:180}")["{invalid_utf8:186}e"]["{invalid_utf8:142}\x19"]("{invalid_utf8:186}e");
                           stop();
                           break loop0;
                        }
                        if(eval("\x01") == 815)
                        {
                           set("\x01",eval("\x01") - 11);
                           §§push(eval(§§pop()));
                        }
                        else if(eval("\x01") == 232)
                        {
                           set("\x01",eval("\x01") + 313);
                           §§push("\x0f");
                           §§push(1);
                        }
                        else if(eval("\x01") == 804)
                        {
                           set("\x01",eval("\x01") + 91);
                           §§push(!§§pop());
                        }
                        else
                        {
                           if(eval("\x01") == 843)
                           {
                              set("\x01",eval("\x01") - 662);
                              break loop0;
                           }
                           if(eval("\x01") == 125)
                           {
                              set("\x01",eval("\x01") + 603);
                           }
                           else
                           {
                              if(eval("\x01") != 895)
                              {
                                 if(eval("\x01") == 34)
                                 {
                                    set("\x01",eval("\x01") - 34);
                                    break loop0;
                                 }
                                 break loop0;
                              }
                              set("\x01",eval("\x01") - 770);
                              if(§§pop())
                              {
                                 set("\x01",eval("\x01") + 603);
                              }
                           }
                        }
                     }
                     while(true)
                     {
                        if(eval("\x01") == 644)
                        {
                           set("\x01",eval("\x01") - 455);
                           §§push(true);
                        }
                        else
                        {
                           if(eval("\x01") != 189)
                           {
                              break;
                           }
                           set("\x01",eval("\x01") + 654);
                           if(§§pop())
                           {
                              set("\x01",eval("\x01") - 662);
                           }
                        }
                     }
                  }
               }
               set("\x01",eval("\x01") - 555);
            }
            set("\x01",eval("\x01") - 334);
            break;
         }
         if(eval("\x01") != 726)
         {
            if(eval("\x01") == 333)
            {
               set("\x01",eval("\x01") - 333);
               break;
            }
            break;
         }
         set("\x01",eval("\x01") - 601);
      }
   }
}
