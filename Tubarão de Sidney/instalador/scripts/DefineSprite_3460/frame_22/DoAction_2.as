function §\x04\x05§()
{
   set("\x03",2282 % 511 * 5);
   return eval("\x03");
}
var §\x01§ = -709 + "\x04\x05"();
loop0:
while(true)
{
   if(eval("\x01") == 481)
   {
      set("\x01",eval("\x01") - 49);
      §§push(true);
      continue;
   }
   if(eval("\x01") == 414)
   {
      set("\x01",eval("\x01") + 156);
      §§push(true);
      continue;
   }
   if(eval("\x01") == 370)
   {
      set("\x01",eval("\x01") + 34);
      eval(§§constant(0))[§§constant(1)][§§constant(2)] += eval(§§constant(3)) * eval(§§constant(0))[§§constant(4)][§§constant(5)];
      break;
   }
   if(eval("\x01") == 432)
   {
      set("\x01",eval("\x01") - 60);
      if(function §\x04\x05§()
      {
         set("\x03",2282 % 511 * 5);
         return eval("\x03");
      })
      {
         set("\x01",eval("\x01") - 336);
      }
      continue;
   }
   if(eval("\x01") == 344)
   {
      set("\x01",eval("\x01") + 70);
      continue;
   }
   if(eval("\x01") == 284)
   {
      set("\x01",eval("\x01") + 130);
      continue;
   }
   if(eval("\x01") == 570)
   {
      set("\x01",eval("\x01") + 315);
      if(function §\x04\x05§()
      {
         set("\x03",2282 % 511 * 5);
         return eval("\x03");
      })
      {
         set("\x01",eval("\x01") - 418);
      }
      continue;
   }
   if(eval("\x01") == 420)
   {
      set("\x01",eval("\x01") - 76);
      break;
   }
   if(eval("\x01") != 372)
   {
      if(eval("\x01") == 260)
      {
         set("\x01",eval("\x01") + 160);
         if(function §\x04\x05§()
         {
            set("\x03",2282 % 511 * 5);
            return eval("\x03");
         })
         {
            set("\x01",eval("\x01") - 76);
         }
      }
      else
      {
         if(eval("\x01") == 885)
         {
            set("\x01",eval("\x01") - 418);
            break;
         }
         if(eval("\x01") == 467)
         {
            set("\x01",eval("\x01") + 481);
         }
         else if(eval("\x01") == 907)
         {
            set("\x01",eval("\x01") - 652);
            §§push(eval(function §\x04\x05§()
            {
               set("\x03",2282 % 511 * 5);
               return eval("\x03");
            }));
         }
         else if(eval("\x01") == 244)
         {
            set("\x01",eval("\x01") + 704);
         }
         else if(eval("\x01") == 948)
         {
            set("\x01",eval("\x01") - 669);
            §§push("\x0f");
            §§push(1);
         }
         else if(eval("\x01") == 36)
         {
            set("\x01",eval("\x01") + 281);
         }
         else if(eval("\x01") == 733)
         {
            set("\x01",eval("\x01") + 174);
            §§push("\x0f");
         }
         else if(eval("\x01") == 279)
         {
            set("\x01",eval("\x01") + 454);
            var §§pop() = function §\x04\x05§()
            {
               set("\x03",2282 % 511 * 5);
               return eval("\x03");
            };
         }
         else if(eval("\x01") == 255)
         {
            set("\x01",eval("\x01") + 70);
            §§push(!function §\x04\x05§()
            {
               set("\x03",2282 % 511 * 5);
               return eval("\x03");
            });
         }
         else if(eval("\x01") == 300)
         {
            set("\x01",eval("\x01") + 70);
         }
         else if(eval("\x01") == 317)
         {
            set("\x01",eval("\x01") - 57);
            §§push(true);
         }
         else if(eval("\x01") == 325)
         {
            set("\x01",eval("\x01") - 25);
            if(function §\x04\x05§()
            {
               set("\x03",2282 % 511 * 5);
               return eval("\x03");
            })
            {
               set("\x01",eval("\x01") + 70);
            }
         }
         else
         {
            if(eval("\x01") != 485)
            {
               if(eval("\x01") == 404)
               {
                  set("\x01",eval("\x01") - 404);
                  break;
               }
               break;
            }
            set("\x01",eval("\x01") - 168);
         }
      }
      continue;
   }
   set("\x01",eval("\x01") - 336);
   while(true)
   {
      if(function §\x04\x05§()
      {
         set("\x03",2282 % 511 * 5);
         return eval("\x03");
      } == 939)
      {
         set("\x01",eval("\x01") - 649);
         §§push(eval(§§pop()));
      }
      else if(eval("\x01") == 419)
      {
         set("\x01",eval("\x01") + 153);
      }
      else
      {
         if(eval("\x01") != 155)
         {
            if(eval("\x01") == 486)
            {
               set("\x01",eval("\x01") - 486);
               break loop0;
            }
            break loop0;
         }
         set("\x01",eval("\x01") + 264);
         if(§§pop())
         {
            set("\x01",eval("\x01") + 153);
         }
      }
      while(true)
      {
         if(eval("\x01") == 418)
         {
            set("\x01",eval("\x01") + 122);
            §§push(true);
            continue;
         }
         if(eval("\x01") == 144)
         {
            set("\x01",eval("\x01") + 569);
            loop3:
            while(true)
            {
               set(§§pop(),§§pop() + §§pop());
               §§push(true);
               while(true)
               {
                  if(eval("\x01") == 951)
                  {
                     set("\x01",eval("\x01") - 161);
                     §§push(true);
                  }
                  else
                  {
                     if(eval("\x01") == 37)
                     {
                        break;
                     }
                     if(eval("\x01") == 369)
                     {
                        set("\x01",eval("\x01") + 186);
                        §§push("\x0f");
                        §§push(1);
                     }
                     else if(eval("\x01") == 790)
                     {
                        set("\x01",eval("\x01") + 75);
                        if(§§pop())
                        {
                           set("\x01",eval("\x01") - 116);
                        }
                     }
                     else
                     {
                        if(eval("\x01") == 865)
                        {
                           set("\x01",eval("\x01") - 116);
                           break loop0;
                        }
                        if(eval("\x01") == 605)
                        {
                           set("\x01",eval("\x01") + 66);
                        }
                        else if(eval("\x01") == 113)
                        {
                           set("\x01",eval("\x01") - 76);
                        }
                        else if(eval("\x01") == 749)
                        {
                           set("\x01",eval("\x01") - 78);
                        }
                        else if(eval("\x01") == 671)
                        {
                           set("\x01",eval("\x01") - 469);
                           §§push(true);
                        }
                        else if(eval("\x01") == 351)
                        {
                           set("\x01",eval("\x01") + 18);
                        }
                        else if(eval("\x01") == 555)
                        {
                           set("\x01",eval("\x01") - 318);
                           var §§pop() = §§pop();
                        }
                        else
                        {
                           if(eval("\x01") == 529)
                           {
                              break loop3;
                           }
                           if(eval("\x01") == 424)
                           {
                              set("\x01",eval("\x01") - 55);
                           }
                           else if(eval("\x01") == 964)
                           {
                              set("\x01",eval("\x01") - 927);
                           }
                           else if(eval("\x01") == 202)
                           {
                              set("\x01",eval("\x01") + 327);
                              if(§§pop())
                              {
                                 set("\x01",eval("\x01") - 416);
                              }
                           }
                           else if(eval("\x01") == 492)
                           {
                              set("\x01",eval("\x01") + 114);
                              if(§§pop())
                              {
                                 set("\x01",eval("\x01") - 182);
                              }
                           }
                           else
                           {
                              if(eval("\x01") == 606)
                              {
                                 set("\x01",eval("\x01") - 182);
                                 break loop0;
                              }
                              if(eval("\x01") == 988)
                              {
                                 set("\x01",eval("\x01") - 353);
                                 §§push(eval(§§pop()));
                              }
                              else if(eval("\x01") == 237)
                              {
                                 set("\x01",eval("\x01") + 751);
                                 §§push("\x0f");
                              }
                              else if(eval("\x01") == 635)
                              {
                                 set("\x01",eval("\x01") - 201);
                                 §§push(!§§pop());
                              }
                              else if(eval("\x01") == 434)
                              {
                                 set("\x01",eval("\x01") - 409);
                                 if(§§pop())
                                 {
                                    set("\x01",eval("\x01") + 63);
                                 }
                              }
                              else
                              {
                                 if(eval("\x01") != 25)
                                 {
                                    if(eval("\x01") == 88)
                                    {
                                       set("\x01",eval("\x01") + 829);
                                       _root.board.score += z * _root.kombi.combo;
                                       break loop0;
                                    }
                                    if(eval("\x01") == 917)
                                    {
                                       set("\x01",eval("\x01") - 917);
                                       break loop0;
                                    }
                                    break loop0;
                                 }
                                 set("\x01",eval("\x01") + 63);
                              }
                           }
                        }
                     }
                  }
               }
            }
            set("\x01",eval("\x01") - 416);
            break loop0;
         }
         if(eval("\x01") == 604)
         {
            set("\x01",eval("\x01") - 151);
            continue;
         }
         if(eval("\x01") == 453)
         {
            set("\x01",eval("\x01") + 324);
            §§push("\x0f");
            §§push(1);
            continue;
         }
         if(eval("\x01") == 915)
         {
            set("\x01",eval("\x01") + 38);
            continue;
         }
         if(eval("\x01") == 879)
         {
            set("\x01",eval("\x01") + 36);
            break loop0;
         }
         if(eval("\x01") != 641)
         {
            if(eval("\x01") == 832)
            {
               set("\x01",eval("\x01") - 192);
            }
            else if(eval("\x01") == 953)
            {
               set("\x01",eval("\x01") - 211);
               §§push(true);
            }
            else if(eval("\x01") == 640)
            {
               set("\x01",eval("\x01") - 376);
               §§push(true);
            }
            else if(eval("\x01") == 713)
            {
               set("\x01",eval("\x01") - 260);
            }
            else if(eval("\x01") == 264)
            {
               set("\x01",eval("\x01") - 120);
               if(§§pop())
               {
                  set("\x01",eval("\x01") + 569);
               }
            }
            else if(eval("\x01") == 540)
            {
               set("\x01",eval("\x01") + 339);
               if(§§pop())
               {
                  set("\x01",eval("\x01") + 36);
               }
            }
            else
            {
               if(eval("\x01") == 572)
               {
                  set("\x01",eval("\x01") - 86);
                  eval("{invalid_utf8:142}{invalid_utf8:238}")["{invalid_utf8:168}\x06O"]["{invalid_utf8:147}V"] += eval("{invalid_utf8:170}5{invalid_utf8:181}{invalid_utf8:240}") * eval("{invalid_utf8:142}{invalid_utf8:238}")[§§constant(4)][§§constant(5)];
                  break loop0;
               }
               if(eval("\x01") == 488)
               {
                  set("\x01",eval("\x01") + 465);
               }
               else if(eval("\x01") == 742)
               {
                  set("\x01",eval("\x01") - 101);
                  if(§§pop())
                  {
                     set("\x01",eval("\x01") - 392);
                  }
               }
               else if(eval("\x01") == 290)
               {
                  set("\x01",eval("\x01") - 135);
                  §§push(!§§pop());
               }
               else if(eval("\x01") == 249)
               {
                  set("\x01",eval("\x01") + 391);
               }
               else if(eval("\x01") == 777)
               {
                  set("\x01",eval("\x01") - 112);
                  var §§pop() = §§pop();
               }
               else
               {
                  if(eval("\x01") != 665)
                  {
                     break;
                  }
                  set("\x01",eval("\x01") + 274);
                  §§push("\x0f");
               }
            }
            continue;
         }
         set("\x01",eval("\x01") - 392);
         loop5:
         while(true)
         {
            if(§§pop() == §§pop())
            {
               set("\x01",eval("\x01") + 394);
            }
            else if(eval("\x01") == 191)
            {
               set("\x01",eval("\x01") - 140);
            }
            else
            {
               if(eval("\x01") == 336)
               {
                  set("\x01",eval("\x01") + 375);
                  eval("\x1c{invalid_utf8:215}")["{{invalid_utf8:183}{invalid_utf8:216}"]["\x10{invalid_utf8:246}"] += eval(§§constant(3)) * eval("\x1c{invalid_utf8:215}")[§§constant(4)][§§constant(5)];
                  break loop0;
               }
               if(eval("\x01") == 939)
               {
                  set("\x01",eval("\x01") - 315);
                  §§push(eval(§§pop()));
               }
               else
               {
                  if(eval("\x01") == 711)
                  {
                     set("\x01",eval("\x01") - 711);
                     break loop0;
                  }
                  if(eval("\x01") == 85)
                  {
                     set("\x01",eval("\x01") + 490);
                     var §§pop() = §§pop();
                  }
                  else if(eval("\x01") == 856)
                  {
                     set("\x01",eval("\x01") - 792);
                     if(§§pop())
                     {
                        set("\x01",eval("\x01") + 127);
                     }
                  }
                  else if(eval("\x01") == 968)
                  {
                     set("\x01",eval("\x01") - 883);
                     §§push("\x0f");
                     §§push(1);
                  }
                  else
                  {
                     if(eval("\x01") == 91)
                     {
                        set("\x01",eval("\x01") + 483);
                        break loop0;
                     }
                     if(eval("\x01") == 64)
                     {
                        set("\x01",eval("\x01") + 127);
                        stop();
                        loop7:
                        while(true)
                        {
                           if(§§pop())
                           {
                              set("\x01",eval("\x01") - 405);
                              var §§pop() = §§pop();
                           }
                           else if(eval("\x01") == 957)
                           {
                              set("\x01",eval("\x01") - 366);
                              if(§§pop())
                              {
                                 set("\x01",eval("\x01") + 86);
                              }
                           }
                           else if(eval("\x01") == 944)
                           {
                              set("\x01",eval("\x01") - 184);
                              if(§§pop())
                              {
                                 set("\x01",eval("\x01") - 666);
                              }
                           }
                           else if(eval("\x01") == 252)
                           {
                              set("\x01",eval("\x01") + 241);
                           }
                           else
                           {
                              if(eval("\x01") == 591)
                              {
                                 set("\x01",eval("\x01") + 86);
                                 break loop0;
                              }
                              if(eval("\x01") == 49)
                              {
                                 set("\x01",eval("\x01") + 490);
                                 §§push(eval(§§pop()));
                              }
                              else if(eval("\x01") == 584)
                              {
                                 set("\x01",eval("\x01") + 116);
                                 §§push("\x0f");
                                 §§push(1);
                              }
                              else if(eval("\x01") == 539)
                              {
                                 set("\x01",eval("\x01") + 405);
                                 §§push(!§§pop());
                              }
                              else if(eval("\x01") == 605)
                              {
                                 set("\x01",eval("\x01") - 21);
                              }
                              else if(eval("\x01") == 295)
                              {
                                 set("\x01",eval("\x01") - 246);
                                 §§push("\x0f");
                              }
                              else
                              {
                                 if(eval("\x01") != 760)
                                 {
                                    if(eval("\x01") == 94)
                                    {
                                       set("\x01",eval("\x01") + 816);
                                       _root.board.score += z * _root.kombi.combo;
                                       break loop0;
                                    }
                                    if(eval("\x01") == 910)
                                    {
                                       set("\x01",eval("\x01") - 910);
                                       break loop0;
                                    }
                                    break loop0;
                                 }
                                 set("\x01",eval("\x01") - 666);
                              }
                           }
                           while(true)
                           {
                              if(eval("\x01") == 285)
                              {
                                 set("\x01",eval("\x01") + 585);
                                 §§push(true);
                                 continue;
                              }
                              if(eval("\x01") == 57)
                              {
                                 set("\x01",eval("\x01") + 436);
                                 continue;
                              }
                              if(eval("\x01") == 677)
                              {
                                 set("\x01",eval("\x01") - 93);
                                 continue;
                              }
                              if(eval("\x01") == 870)
                              {
                                 set("\x01",eval("\x01") + 1);
                                 if(§§pop())
                                 {
                                    set("\x01",eval("\x01") - 814);
                                 }
                                 continue;
                              }
                              if(eval("\x01") == 871)
                              {
                                 break;
                              }
                              if(eval("\x01") == 493)
                              {
                                 set("\x01",eval("\x01") + 464);
                                 §§push(true);
                                 continue;
                              }
                              continue loop7;
                           }
                           set("\x01",eval("\x01") - 814);
                           break loop0;
                        }
                     }
                     else if(eval("\x01") == 51)
                     {
                        set("\x01",eval("\x01") + 528);
                        §§push(true);
                     }
                     else
                     {
                        if(eval("\x01") != 196)
                        {
                           break loop0;
                        }
                        set("\x01",eval("\x01") - 145);
                     }
                  }
               }
            }
            while(true)
            {
               if(eval("\x01") == 652)
               {
                  set("\x01",eval("\x01") + 204);
                  §§push(true);
                  continue;
               }
               if(eval("\x01") == 887)
               {
                  set("\x01",eval("\x01") - 289);
                  continue;
               }
               if(eval("\x01") == 575)
               {
                  set("\x01",eval("\x01") + 364);
                  §§push("\x0f");
                  continue;
               }
               if(eval("\x01") == 579)
               {
                  set("\x01",eval("\x01") - 259);
                  if(§§pop())
                  {
                     set("\x01",eval("\x01") + 567);
                  }
                  continue;
               }
               if(eval("\x01") == 839)
               {
                  set("\x01",eval("\x01") - 263);
                  if(§§pop())
                  {
                     set("\x01",eval("\x01") - 240);
                  }
                  continue;
               }
               if(eval("\x01") == 444)
               {
                  set("\x01",eval("\x01") + 524);
                  continue;
               }
               if(eval("\x01") == 99)
               {
                  set("\x01",eval("\x01") + 499);
                  continue;
               }
               if(eval("\x01") == 576)
               {
                  set("\x01",eval("\x01") - 240);
                  continue;
               }
               if(eval("\x01") == 320)
               {
                  break;
               }
               if(eval("\x01") == 598)
               {
                  set("\x01",eval("\x01") - 238);
                  §§push(true);
                  continue;
               }
               if(eval("\x01") == 360)
               {
                  set("\x01",eval("\x01") - 269);
                  if(§§pop())
                  {
                     set("\x01",eval("\x01") + 483);
                  }
                  continue;
               }
               if(eval("\x01") == 624)
               {
                  set("\x01",eval("\x01") + 215);
                  §§push(!§§pop());
                  continue;
               }
               continue loop5;
            }
            set("\x01",eval("\x01") + 567);
            break loop0;
         }
      }
   }
}
