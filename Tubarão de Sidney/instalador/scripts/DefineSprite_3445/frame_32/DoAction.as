function §\x04\x05§()
{
   set("\x03",889 % 511 * 5);
   return eval("\x03");
}
var §\x01§ = -1011 + "\x04\x05"();
loop0:
while(true)
{
   if(eval("\x01") == 879)
   {
      set("\x01",eval("\x01") - 829);
      §§push(true);
   }
   else if(eval("\x01") == 165)
   {
      set("\x01",eval("\x01") + 190);
   }
   else
   {
      if(eval("\x01") == 798)
      {
         set("\x01",eval("\x01") - 532);
         set(§§pop(),function §\x04\x05§()
         {
            set("\x03",889 % 511 * 5);
            return eval("\x03");
         });
         break;
      }
      if(eval("\x01") == 823)
      {
         set("\x01",eval("\x01") - 25);
         if(function §\x04\x05§()
         {
            set("\x03",889 % 511 * 5);
            return eval("\x03");
         })
         {
            set("\x01",eval("\x01") - 532);
         }
      }
      else if(eval("\x01") == 685)
      {
         set("\x01",eval("\x01") - 472);
      }
      else if(eval("\x01") == 355)
      {
         set("\x01",eval("\x01") + 468);
         §§push(true);
      }
      else if(eval("\x01") == 910)
      {
         set("\x01",eval("\x01") - 460);
         var §§pop() = function §\x04\x05§()
         {
            set("\x03",889 % 511 * 5);
            return eval("\x03");
         };
      }
      else if(eval("\x01") == 158)
      {
         set("\x01",eval("\x01") + 197);
      }
      else if(eval("\x01") == 450)
      {
         set("\x01",eval("\x01") + 350);
         §§push("\x0f");
      }
      else if(eval("\x01") == 50)
      {
         set("\x01",eval("\x01") + 124);
         if(function §\x04\x05§()
         {
            set("\x03",889 % 511 * 5);
            return eval("\x03");
         })
         {
            set("\x01",eval("\x01") - 16);
         }
      }
      else
      {
         if(eval("\x01") == 174)
         {
            set("\x01",eval("\x01") - 16);
            loop1:
            while(true)
            {
               set(function §\x04\x05§()
               {
                  set("\x03",889 % 511 * 5);
                  return eval("\x03");
               },eval("\x01") + 84);
               while(true)
               {
                  if(eval("\x01") == 92)
                  {
                     set("\x01",eval("\x01") + 535);
                     §§push(true);
                     continue;
                  }
                  if(eval("\x01") == 227)
                  {
                     set("\x01",eval("\x01") + 462);
                     if(§§pop())
                     {
                        set("\x01",eval("\x01") - 391);
                     }
                     continue;
                  }
                  if(eval("\x01") == 862)
                  {
                     set("\x01",eval("\x01") - 638);
                     continue;
                  }
                  if(eval("\x01") == 921)
                  {
                     set("\x01",eval("\x01") - 379);
                     if(§§pop())
                     {
                        set("\x01",eval("\x01") + 84);
                     }
                     continue;
                  }
                  if(eval("\x01") == 378)
                  {
                     set("\x01",eval("\x01") + 484);
                     set(§§pop(),eval("\x01") - 25);
                     addr198:
                     if(§§pop())
                     {
                        set("\x01",eval("\x01") + 81);
                        addr19:
                        §§push("\x0f");
                        if(eval("\x01") == 505)
                        {
                           set("\x01",eval("\x01") + 415);
                           §§goto(addr19);
                           §§push(true);
                        }
                        if(eval("\x01") == 788)
                        {
                           set("\x01",eval("\x01") - 492);
                           §§goto(addr19);
                        }
                        if(eval("\x01") == 22)
                        {
                           set("\x01",eval("\x01") + 766);
                           if(!§§pop())
                           {
                              addr70:
                              §§goto(addr19);
                           }
                           set("\x01",eval("\x01") - 492);
                           §§goto(addr70);
                        }
                        if(eval("\x01") == 343)
                        {
                           set("\x01",eval("\x01") + 499);
                           §§goto(addr19);
                           §§push(eval(§§pop()));
                        }
                        if(eval("\x01") != 247)
                        {
                           if(eval("\x01") == 461)
                           {
                              set("\x01",eval("\x01") - 266);
                              §§goto(addr19);
                           }
                           if(eval("\x01") == 386)
                           {
                              set("\x01",eval("\x01") - 191);
                              §§goto(addr19);
                           }
                           if(eval("\x01") != 296)
                           {
                              if(eval("\x01") == 842)
                              {
                                 set("\x01",eval("\x01") - 820);
                                 §§goto(addr19);
                                 §§push(!§§pop());
                              }
                              if(eval("\x01") == 195)
                              {
                                 set("\x01",eval("\x01") - 130);
                                 §§goto(addr19);
                                 §§push("\x0f");
                                 §§push(1);
                              }
                              if(eval("\x01") != 317)
                              {
                                 if(eval("\x01") == 65)
                                 {
                                    set("\x01",eval("\x01") + 197);
                                    var §§pop() = §§pop();
                                    §§goto(addr19);
                                 }
                                 §§goto(addr198);
                                 §§push(eval("\x01") == 262);
                              }
                              set("\x01",eval("\x01") - 317);
                              break loop0;
                           }
                           set("\x01",eval("\x01") + 21);
                           eval("\x19`{invalid_utf8:195}{invalid_utf8:246}")["{invalid_utf8:188}{invalid_utf8:181}"]["$d{invalid_utf8:169}"] -= eval("{invalid_utf8:132}r@E");
                           break loop0;
                        }
                        set("\x01",eval("\x01") + 214);
                        break loop0;
                     }
                     if(eval("\x01") == 920)
                     {
                        set("\x01",eval("\x01") - 673);
                        if(!§§pop())
                        {
                           addr228:
                           §§goto(addr19);
                        }
                        set("\x01",eval("\x01") + 214);
                        §§goto(addr228);
                     }
                     break loop0;
                  }
                  if(eval("\x01") == 760)
                  {
                     set("\x01",eval("\x01") - 533);
                     §§push(true);
                     continue;
                  }
                  if(eval("\x01") == 209)
                  {
                     set("\x01",eval("\x01") + 712);
                     §§push(!§§pop());
                     continue;
                  }
                  if(eval("\x01") == 372)
                  {
                     set("\x01",eval("\x01") + 87);
                     §§push(true);
                     continue;
                  }
                  if(eval("\x01") == 234)
                  {
                     set("\x01",eval("\x01") - 25);
                     §§push(eval(§§pop()));
                     continue;
                  }
                  if(eval("\x01") == 459)
                  {
                     set("\x01",eval("\x01") - 81);
                     if(§§pop())
                     {
                        set("\x01",eval("\x01") + 484);
                     }
                     continue;
                  }
                  if(eval("\x01") == 348)
                  {
                     set("\x01",eval("\x01") + 24);
                     continue;
                  }
                  if(eval("\x01") == 396)
                  {
                     set("\x01",eval("\x01") - 172);
                     continue;
                  }
                  if(eval("\x01") == 627)
                  {
                     set("\x01",eval("\x01") - 573);
                     if(§§pop())
                     {
                        set("\x01",eval("\x01") + 875);
                     }
                     continue;
                  }
                  if(eval("\x01") == 339)
                  {
                     set("\x01",eval("\x01") - 105);
                     §§push("\x0f");
                     continue;
                  }
                  if(eval("\x01") == 689)
                  {
                     break loop1;
                  }
                  if(eval("\x01") == 364)
                  {
                     set("\x01",eval("\x01") - 25);
                     var §§pop() = §§pop();
                     continue;
                  }
                  if(eval("\x01") == 929)
                  {
                     set("\x01",eval("\x01") - 169);
                     continue;
                  }
                  if(eval("\x01") == 54)
                  {
                     set("\x01",eval("\x01") + 875);
                     startDrag(§§pop(),§§pop(),§§pop(),§§pop(),§§pop(),§§pop());
                     break loop0;
                  }
                  if(eval("\x01") == 237)
                  {
                     set("\x01",eval("\x01") + 523);
                     continue;
                  }
                  if(eval("\x01") == 298)
                  {
                     set("\x01",eval("\x01") + 74);
                     continue;
                  }
                  if(eval("\x01") == 224)
                  {
                     set("\x01",eval("\x01") + 140);
                     §§push("\x0f");
                     §§push(1);
                     continue;
                  }
                  if(eval("\x01") != 542)
                  {
                     if(eval("\x01") == 626)
                     {
                        set("\x01",eval("\x01") - 146);
                        eval("{invalid_utf8:240}{invalid_utf8:196}")[":{invalid_utf8:236}f"]["k-"] -= eval("{invalid_utf8:148}{invalid_utf8:165}");
                        eval("{invalid_utf8:240}{invalid_utf8:196}")[":{invalid_utf8:236}f"][§§constant(4)] -= eval("{invalid_utf8:148}{invalid_utf8:165}");
                        break loop0;
                     }
                     if(eval("\x01") == 480)
                     {
                        set("\x01",eval("\x01") - 480);
                        break loop0;
                     }
                     break loop0;
                  }
                  continue loop1;
               }
            }
            set("\x01",eval("\x01") - 391);
            break;
         }
         if(eval("\x01") == 800)
         {
            set("\x01",eval("\x01") - 589);
            §§push(eval(function §\x04\x05§()
            {
               set("\x03",889 % 511 * 5);
               return eval("\x03");
            }));
         }
         else if(eval("\x01") == 955)
         {
            set("\x01",eval("\x01") - 632);
         }
         else if(eval("\x01") == 266)
         {
            set("\x01",eval("\x01") + 57);
         }
         else if(eval("\x01") == 211)
         {
            set("\x01",eval("\x01") + 647);
            §§push(!function §\x04\x05§()
            {
               set("\x03",889 % 511 * 5);
               return eval("\x03");
            });
         }
         else if(eval("\x01") == 858)
         {
            set("\x01",eval("\x01") - 173);
            if(function §\x04\x05§()
            {
               set("\x03",889 % 511 * 5);
               return eval("\x03");
            })
            {
               set("\x01",eval("\x01") - 472);
            }
         }
         else
         {
            if(eval("\x01") == 213)
            {
               set("\x01",eval("\x01") + 74);
               eval("{invalid_utf8:152}\x17")["{invalid_utf8:185}{invalid_utf8:134}"]["{invalid_utf8:198}\x1fW"] += eval("{invalid_utf8:220}\x12 q");
               break;
            }
            if(eval("\x01") != 323)
            {
               if(eval("\x01") == 287)
               {
                  set("\x01",eval("\x01") - 287);
                  break;
               }
               break;
            }
            set("\x01",eval("\x01") + 587);
            §§push("\x0f");
            §§push(1);
         }
      }
   }
}
