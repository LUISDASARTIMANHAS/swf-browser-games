function §\x04\x05§()
{
   set("\x03",721 % 511 * 5);
   return eval("\x03");
}
var §\x01§ = -265 + "\x04\x05"();
loop0:
while(true)
{
   if(eval("\x01") == 785)
   {
      set("\x01",eval("\x01") - 180);
      §§push(true);
      continue;
   }
   if(eval("\x01") != 369)
   {
      if(eval("\x01") == 851)
      {
         set("\x01",eval("\x01") - 584);
         §§push(eval(function §\x04\x05§()
         {
            set("\x03",721 % 511 * 5);
            return eval("\x03");
         }));
      }
      else if(eval("\x01") == 165)
      {
         set("\x01",eval("\x01") - 18);
         var §§pop() = function §\x04\x05§()
         {
            set("\x03",721 % 511 * 5);
            return eval("\x03");
         };
      }
      else if(eval("\x01") == 605)
      {
         set("\x01",eval("\x01") - 236);
         if(function §\x04\x05§()
         {
            set("\x03",721 % 511 * 5);
            return eval("\x03");
         })
         {
            set("\x01",eval("\x01") + 323);
         }
      }
      else if(eval("\x01") == 692)
      {
         set("\x01",eval("\x01") + 7);
      }
      else if(eval("\x01") == 973)
      {
         set("\x01",eval("\x01") - 274);
      }
      else if(eval("\x01") == 699)
      {
         set("\x01",eval("\x01") - 534);
         §§push("\x0f");
         §§push(1);
      }
      else if(eval("\x01") == 147)
      {
         set("\x01",eval("\x01") + 704);
         §§push("\x0f");
      }
      else if(eval("\x01") == 267)
      {
         set("\x01",eval("\x01") + 483);
         §§push(!function §\x04\x05§()
         {
            set("\x03",721 % 511 * 5);
            return eval("\x03");
         });
      }
      else if(eval("\x01") == 750)
      {
         set("\x01",eval("\x01") + 204);
         if(function §\x04\x05§()
         {
            set("\x03",721 % 511 * 5);
            return eval("\x03");
         })
         {
            set("\x01",eval("\x01") - 786);
         }
      }
      else
      {
         if(eval("\x01") != 954)
         {
            if(eval("\x01") == 168)
            {
               set("\x01",eval("\x01") + 456);
               eval("-{invalid_utf8:134}{invalid_utf8:134}")["{invalid_utf8:178}W\x19"]["~{invalid_utf8:221}"] -= eval(§§constant(3));
               break;
            }
            if(eval("\x01") == 624)
            {
               set("\x01",eval("\x01") - 624);
               break;
            }
            break;
         }
         set("\x01",eval("\x01") - 786);
      }
      continue;
   }
   set("\x01",eval("\x01") + 323);
   §§push(§§pop() eq function §\x04\x05§()
   {
      set("\x03",721 % 511 * 5);
      return eval("\x03");
   });
   loop1:
   while(true)
   {
      if(§§pop() == §§pop())
      {
         set("\x01",eval("\x01") + 437);
         set("p\x11{invalid_utf8:160}",8);
         set("={invalid_utf8:146}",10);
         set(§§constant(2),10);
         set(§§constant(3),eval(§§constant(4))[§§constant(5)][§§constant(6)]);
         set(§§constant(7),eval(§§constant(4))[§§constant(5)][§§constant(8)]);
         break loop0;
      }
      if(eval("\x01") == 100)
      {
         set("\x01",eval("\x01") + 589);
         if(§§pop())
         {
            set("\x01",eval("\x01") - 206);
         }
      }
      else if(eval("\x01") == 976)
      {
         set("\x01",eval("\x01") - 501);
      }
      else
      {
         if(eval("\x01") == 676)
         {
            set("\x01",eval("\x01") + 300);
            break loop0;
         }
         if(eval("\x01") == 287)
         {
            set("\x01",eval("\x01") - 153);
            var §§pop() = §§pop();
         }
         else if(eval("\x01") == 134)
         {
            set("\x01",eval("\x01") - 68);
            §§push("\x0f");
         }
         else
         {
            if(eval("\x01") == 865)
            {
               set("\x01",eval("\x01") - 865);
               break loop0;
            }
            if(eval("\x01") == 205)
            {
               set("\x01",eval("\x01") + 322);
               §§push(!§§pop());
            }
            else
            {
               if(eval("\x01") == 551)
               {
                  set("\x01",eval("\x01") + 247);
                  break loop0;
               }
               if(eval("\x01") != 66)
               {
                  break loop0;
               }
               set("\x01",eval("\x01") + 139);
               §§push(eval(§§pop()));
            }
         }
      }
      while(true)
      {
         if(eval("\x01") == 826)
         {
            set("\x01",eval("\x01") - 726);
            §§push(true);
            continue;
         }
         if(eval("\x01") == 678)
         {
            set("\x01",eval("\x01") - 250);
            continue;
         }
         if(eval("\x01") == 483)
         {
            set("\x01",eval("\x01") - 332);
            continue;
         }
         if(eval("\x01") == 441)
         {
            set("\x01",eval("\x01") + 110);
            if(§§pop())
            {
               set("\x01",eval("\x01") + 247);
            }
            continue;
         }
         if(eval("\x01") == 151)
         {
            set("\x01",eval("\x01") + 290);
            §§push(true);
            continue;
         }
         if(eval("\x01") == 694)
         {
            set("\x01",eval("\x01") - 115);
            continue;
         }
         if(eval("\x01") == 751)
         {
            set("\x01",eval("\x01") - 600);
            continue;
         }
         if(eval("\x01") == 475)
         {
            set("\x01",eval("\x01") - 188);
            §§push("\x0f");
            §§push(1);
            continue;
         }
         if(eval("\x01") == 798)
         {
            set("\x01",eval("\x01") - 219);
            continue;
         }
         if(eval("\x01") == 580)
         {
            set("\x01",eval("\x01") - 105);
            continue;
         }
         if(eval("\x01") == 579)
         {
            set("\x01",eval("\x01") + 201);
            §§push(true);
            continue;
         }
         if(eval("\x01") == 527)
         {
            set("\x01",eval("\x01") + 151);
            if(§§pop())
            {
               set("\x01",eval("\x01") - 250);
            }
            continue;
         }
         if(eval("\x01") == 689)
         {
            break;
         }
         if(eval("\x01") == 780)
         {
            set("\x01",eval("\x01") - 104);
            if(§§pop())
            {
               set("\x01",eval("\x01") + 300);
            }
            continue;
         }
         continue loop1;
      }
      set("\x01",eval("\x01") - 206);
      loop3:
      while(true)
      {
         set(§§pop(),§§pop());
         §§push(!§§pop());
         while(true)
         {
            if(eval("\x01") == 632)
            {
               set("\x01",eval("\x01") - 131);
               §§push(true);
            }
            else if(eval("\x01") == 137)
            {
               set("\x01",eval("\x01") + 263);
               §§push("\x0f");
            }
            else if(eval("\x01") == 26)
            {
               set("\x01",eval("\x01") + 665);
            }
            else
            {
               if(eval("\x01") == 861)
               {
                  break;
               }
               if(eval("\x01") == 116)
               {
                  set("\x01",eval("\x01") + 224);
               }
               else if(eval("\x01") == 691)
               {
                  set("\x01",eval("\x01") - 301);
                  §§push("\x0f");
                  §§push(1);
               }
               else if(eval("\x01") == 501)
               {
                  set("\x01",eval("\x01") - 167);
                  if(§§pop())
                  {
                     set("\x01",eval("\x01") - 218);
                  }
               }
               else if(eval("\x01") == 390)
               {
                  set("\x01",eval("\x01") - 253);
                  var §§pop() = §§pop();
               }
               else if(eval("\x01") == 738)
               {
                  set("\x01",eval("\x01") - 359);
                  if(§§pop())
                  {
                     set("\x01",eval("\x01") - 304);
                  }
               }
               else
               {
                  if(eval("\x01") == 334)
                  {
                     set("\x01",eval("\x01") - 218);
                     break loop0;
                  }
                  if(eval("\x01") == 379)
                  {
                     set("\x01",eval("\x01") - 304);
                  }
                  else
                  {
                     if(eval("\x01") == 128)
                     {
                        break loop3;
                     }
                     if(eval("\x01") == 75)
                     {
                        set("\x01",eval("\x01") + 378);
                        _root.board.score += z;
                        break loop0;
                     }
                     if(eval("\x01") == 453)
                     {
                        set("\x01",eval("\x01") - 453);
                        break loop0;
                     }
                     if(eval("\x01") == 400)
                     {
                        set("\x01",eval("\x01") + 461);
                        §§push(eval(§§pop()));
                     }
                     else if(eval("\x01") == 213)
                     {
                        set("\x01",eval("\x01") + 127);
                     }
                     else if(eval("\x01") == 842)
                     {
                        set("\x01",eval("\x01") - 151);
                     }
                     else if(eval("\x01") == 87)
                     {
                        set("\x01",eval("\x01") + 41);
                        if(§§pop())
                        {
                           set("\x01",eval("\x01") + 714);
                        }
                     }
                     else
                     {
                        if(eval("\x01") != 340)
                        {
                           break loop0;
                        }
                        set("\x01",eval("\x01") - 253);
                        §§push(true);
                     }
                  }
               }
            }
         }
      }
      set("\x01",eval("\x01") + 714);
      break loop0;
   }
}
