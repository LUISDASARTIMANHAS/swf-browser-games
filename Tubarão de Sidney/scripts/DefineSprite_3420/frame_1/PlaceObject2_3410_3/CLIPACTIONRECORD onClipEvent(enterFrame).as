onClipEvent(enterFrame){
   function §\x04\x05§()
   {
      set("\x03",2251 % 511 * 5);
      return eval("\x03");
   }
   var §\x01§ = -465 + "\x04\x05"();
   loop0:
   while(true)
   {
      if(eval("\x01") == 570)
      {
         set("\x01",eval("\x01") - 297);
         §§push(true);
         continue;
      }
      if(eval("\x01") == 492)
      {
         set("\x01",eval("\x01") + 280);
         continue;
      }
      if(eval("\x01") == 507)
      {
         set("\x01",eval("\x01") + 216);
         §§push(eval(function §\x04\x05§()
         {
            set("\x03",2251 % 511 * 5);
            return eval("\x03");
         }));
         continue;
      }
      if(eval("\x01") == 997)
      {
         set("\x01",eval("\x01") - 232);
         continue;
      }
      if(eval("\x01") == 94)
      {
         set("\x01",eval("\x01") + 671);
         continue;
      }
      if(eval("\x01") == 461)
      {
         set("\x01",eval("\x01") + 46);
         §§push("\x0f");
         continue;
      }
      if(eval("\x01") == 765)
      {
         set("\x01",eval("\x01") - 683);
         §§push("\x0f");
         §§push(1);
         continue;
      }
      if(eval("\x01") == 273)
      {
         set("\x01",eval("\x01") + 282);
         if(function §\x04\x05§()
         {
            set("\x03",2251 % 511 * 5);
            return eval("\x03");
         })
         {
            set("\x01",eval("\x01") - 461);
         }
         continue;
      }
      if(eval("\x01") == 82)
      {
         set("\x01",eval("\x01") + 379);
         var §§pop() = function §\x04\x05§()
         {
            set("\x03",2251 % 511 * 5);
            return eval("\x03");
         };
         continue;
      }
      if(eval("\x01") != 555)
      {
         if(eval("\x01") == 723)
         {
            set("\x01",eval("\x01") - 628);
            §§push(!function §\x04\x05§()
            {
               set("\x03",2251 % 511 * 5);
               return eval("\x03");
            });
         }
         else
         {
            if(eval("\x01") != 95)
            {
               if(eval("\x01") == 772)
               {
                  set("\x01",eval("\x01") + 199);
                  if(eval(§§constant(7))[§§constant(8)](eval("*{invalid_utf8:159}\x183")["{invalid_utf8:187}\x0f{invalid_utf8:184}"][§§constant(2)][§§constant(3)][§§constant(4)][§§constant(5)][§§constant(6)]))
                  {
                     eval("*{invalid_utf8:159}\x183")[§§constant(9)] = 1;
                     eval("*{invalid_utf8:159}\x183")[§§constant(10)] = 1;
                     eval("*{invalid_utf8:159}\x183")["{invalid_utf8:187}\x0f{invalid_utf8:184}"][§§constant(2)][§§constant(3)][§§constant(12)][§§constant(13)](§§constant(11));
                  }
                  break;
               }
               if(eval("\x01") == 971)
               {
                  set("\x01",eval("\x01") - 971);
                  break;
               }
               break;
            }
            set("\x01",eval("\x01") + 397);
            if(function §\x04\x05§()
            {
               set("\x03",2251 % 511 * 5);
               return eval("\x03");
            })
            {
               set("\x01",eval("\x01") + 280);
            }
         }
         continue;
      }
      set("\x01",eval("\x01") - 461);
      while(true)
      {
         if(§§pop() == function §\x04\x05§()
         {
            set("\x03",2251 % 511 * 5);
            return eval("\x03");
         })
         {
            set("\x01",eval("\x01") + 499);
            §§push("\x0f");
         }
         else if(eval("\x01") == 921)
         {
            set("\x01",eval("\x01") - 455);
            if(§§pop())
            {
               set("\x01",eval("\x01") - 137);
            }
         }
         else if(eval("\x01") == 329)
         {
            set("\x01",eval("\x01") - 96);
         }
         else
         {
            if(eval("\x01") == 362)
            {
               set("\x01",eval("\x01") - 362);
               break loop0;
            }
            if(eval("\x01") != 283)
            {
               if(eval("\x01") == 466)
               {
                  set("\x01",eval("\x01") - 137);
                  loop3:
                  while(true)
                  {
                     set(§§pop(),eval(§§pop()) - 104);
                     if(§§pop())
                     {
                        set("\x01",eval("\x01") + 1);
                     }
                     while(true)
                     {
                        if(eval("\x01") == 86)
                        {
                           set("\x01",eval("\x01") + 885);
                           §§push(true);
                        }
                        else if(eval("\x01") == 308)
                        {
                           set("\x01",eval("\x01") - 279);
                        }
                        else
                        {
                           if(eval("\x01") == 612)
                           {
                              break loop3;
                           }
                           if(eval("\x01") == 269)
                           {
                              set("\x01",eval("\x01") + 252);
                              if(this.hitTest(_root.game.inner.inner2.shark))
                              {
                                 if(_root.breaker == 1)
                                 {
                                    _root.breaker = 0;
                                    _root.text2 = _root.text1;
                                    _root.text1 = _root.text0;
                                    _root.text0 = "SHARK CRUSHED SURFER";
                                    _root.glow.gotoAndPlay(2);
                                    _root.game.inner.inner2.surfer.boy2.inner.inner2.gotoAndPlay("tod");
                                 }
                              }
                              break loop0;
                           }
                           if(eval("\x01") == 168)
                           {
                              set("\x01",eval("\x01") - 119);
                           }
                           else if(eval("\x01") == 268)
                           {
                              set("\x01",eval("\x01") + 1);
                           }
                           else
                           {
                              if(eval("\x01") == 84)
                              {
                                 set("\x01",eval("\x01") + 224);
                                 loop5:
                                 while(true)
                                 {
                                    set(§§pop(),§§pop());
                                    while(true)
                                    {
                                       if(eval("\x01") == 942)
                                       {
                                          set("\x01",eval("\x01") - 691);
                                          §§push(true);
                                       }
                                       else
                                       {
                                          if(eval("\x01") == 264)
                                          {
                                             break loop5;
                                          }
                                          if(eval("\x01") == 836)
                                          {
                                             set("\x01",eval("\x01") - 346);
                                          }
                                          else if(eval("\x01") == 238)
                                          {
                                             set("\x01",eval("\x01") + 277);
                                             var §§pop() = §§pop();
                                          }
                                          else if(eval("\x01") == 643)
                                          {
                                             set("\x01",eval("\x01") - 310);
                                             §§push(!§§pop());
                                          }
                                          else if(eval("\x01") == 994)
                                          {
                                             set("\x01",eval("\x01") - 932);
                                          }
                                          else if(eval("\x01") == 490)
                                          {
                                             set("\x01",eval("\x01") - 252);
                                             §§push("\x0f");
                                             §§push(1);
                                          }
                                          else
                                          {
                                             if(eval("\x01") == 682)
                                             {
                                                set("\x01",eval("\x01") - 109);
                                                break loop0;
                                             }
                                             if(eval("\x01") == 696)
                                             {
                                                break;
                                             }
                                             if(eval("\x01") == 251)
                                             {
                                                set("\x01",eval("\x01") - 132);
                                                if(§§pop())
                                                {
                                                   set("\x01",eval("\x01") + 577);
                                                }
                                             }
                                             else if(eval("\x01") == 595)
                                             {
                                                set("\x01",eval("\x01") + 48);
                                                §§push(eval(§§pop()));
                                             }
                                             else if(eval("\x01") == 367)
                                             {
                                                set("\x01",eval("\x01") - 103);
                                             }
                                             else
                                             {
                                                if(eval("\x01") == 119)
                                                {
                                                   set("\x01",eval("\x01") + 577);
                                                   break loop0;
                                                }
                                                if(eval("\x01") == 8)
                                                {
                                                   set("\x01",eval("\x01") + 674);
                                                   if(§§pop())
                                                   {
                                                      set("\x01",eval("\x01") - 109);
                                                   }
                                                }
                                                else if(eval("\x01") == 62)
                                                {
                                                   set("\x01",eval("\x01") - 54);
                                                   §§push(true);
                                                }
                                                else if(eval("\x01") == 573)
                                                {
                                                   set("\x01",eval("\x01") - 83);
                                                }
                                                else if(eval("\x01") == 515)
                                                {
                                                   set("\x01",eval("\x01") + 80);
                                                   §§push("\x0f");
                                                }
                                                else
                                                {
                                                   if(eval("\x01") != 333)
                                                   {
                                                      if(eval("\x01") == 798)
                                                      {
                                                         set("\x01",eval("\x01") - 798);
                                                         break loop0;
                                                      }
                                                      break loop0;
                                                   }
                                                   set("\x01",eval("\x01") + 34);
                                                   if(§§pop())
                                                   {
                                                      set("\x01",eval("\x01") - 103);
                                                   }
                                                }
                                             }
                                          }
                                       }
                                    }
                                 }
                                 set("\x01",eval("\x01") + 534);
                                 eval("{invalid_utf8:144}{invalid_utf8:232}{invalid_utf8:227}{invalid_utf8:221}")["`uX"]["{invalid_utf8:229}\x1e"] += eval("{invalid_utf8:247}{invalid_utf8:239}") * eval("{invalid_utf8:144}{invalid_utf8:232}{invalid_utf8:227}{invalid_utf8:221}")[§§constant(4)][§§constant(5)];
                                 break loop0;
                              }
                              if(eval("\x01") == 96)
                              {
                                 set("\x01",eval("\x01") - 67);
                              }
                              else if(eval("\x01") == 49)
                              {
                                 set("\x01",eval("\x01") + 774);
                                 §§push(true);
                              }
                              else if(eval("\x01") == 532)
                              {
                                 set("\x01",eval("\x01") - 483);
                              }
                              else
                              {
                                 if(eval("\x01") == 372)
                                 {
                                    break;
                                 }
                                 if(eval("\x01") == 521)
                                 {
                                    set("\x01",eval("\x01") - 521);
                                    break loop0;
                                 }
                                 if(eval("\x01") == 971)
                                 {
                                    set("\x01",eval("\x01") - 359);
                                    if(§§pop())
                                    {
                                       set("\x01",eval("\x01") - 80);
                                    }
                                 }
                                 else if(eval("\x01") == 823)
                                 {
                                    set("\x01",eval("\x01") - 739);
                                    if(§§pop())
                                    {
                                       set("\x01",eval("\x01") + 224);
                                    }
                                 }
                                 else if(eval("\x01") == 938)
                                 {
                                    set("\x01",eval("\x01") - 566);
                                    §§push(!§§pop());
                                 }
                                 else if(eval("\x01") == 169)
                                 {
                                    set("\x01",eval("\x01") - 128);
                                    var §§pop() = §§pop();
                                 }
                                 else if(eval("\x01") == 29)
                                 {
                                    set("\x01",eval("\x01") + 140);
                                    §§push("\x0f");
                                    §§push(1);
                                 }
                                 else if(eval("\x01") == 41)
                                 {
                                    set("\x01",eval("\x01") + 402);
                                    §§push("\x0f");
                                 }
                                 else
                                 {
                                    if(eval("\x01") != 443)
                                    {
                                       break loop0;
                                    }
                                    set("\x01",eval("\x01") + 495);
                                    §§push(eval(§§pop()));
                                 }
                              }
                           }
                        }
                     }
                  }
                  set("\x01",eval("\x01") - 80);
                  break loop0;
               }
               break loop0;
            }
            set("\x01",eval("\x01") - 50);
         }
         while(true)
         {
            if(eval("\x01") == 140)
            {
               set("\x01",eval("\x01") + 781);
               §§push(true);
            }
            else if(eval("\x01") == 138)
            {
               set("\x01",eval("\x01") + 24);
            }
            else
            {
               if(eval("\x01") == 220)
               {
                  set("\x01",eval("\x01") - 82);
                  break loop0;
               }
               if(eval("\x01") == 643)
               {
                  set("\x01",eval("\x01") + 297);
               }
               else if(eval("\x01") == 341)
               {
                  set("\x01",eval("\x01") - 179);
               }
               else if(eval("\x01") == 109)
               {
                  set("\x01",eval("\x01") + 321);
                  if(§§pop())
                  {
                     set("\x01",eval("\x01") + 99);
                  }
               }
               else if(eval("\x01") == 285)
               {
                  set("\x01",eval("\x01") + 358);
                  if(§§pop())
                  {
                     set("\x01",eval("\x01") + 297);
                  }
               }
               else if(eval("\x01") == 273)
               {
                  set("\x01",eval("\x01") + 376);
                  §§push(true);
               }
               else if(eval("\x01") == 924)
               {
                  set("\x01",eval("\x01") - 36);
                  §§push(eval(§§pop()));
               }
               else if(eval("\x01") == 233)
               {
                  set("\x01",eval("\x01") - 124);
                  §§push(true);
               }
               else if(eval("\x01") == 649)
               {
                  set("\x01",eval("\x01") - 429);
                  if(§§pop())
                  {
                     set("\x01",eval("\x01") - 82);
                  }
               }
               else if(eval("\x01") == 766)
               {
                  set("\x01",eval("\x01") - 493);
               }
               else if(eval("\x01") == 888)
               {
                  set("\x01",eval("\x01") - 603);
                  §§push(!§§pop());
               }
               else if(eval("\x01") == 162)
               {
                  set("\x01",eval("\x01") + 70);
                  §§push("\x0f");
                  §§push(1);
               }
               else
               {
                  if(eval("\x01") == 940)
                  {
                     set("\x01",eval("\x01") - 578);
                     eval("{invalid_utf8:133}{invalid_utf8:211}")[":x{invalid_utf8:255}"][§§constant(2)][§§constant(3)][§§constant(4)][§§constant(5)] = eval("{invalid_utf8:133}{invalid_utf8:211}")[":x{invalid_utf8:255}"][§§constant(2)][§§constant(3)][§§constant(6)][§§constant(5)];
                     break loop0;
                  }
                  if(eval("\x01") == 232)
                  {
                     set("\x01",eval("\x01") + 193);
                     var §§pop() = §§pop();
                  }
                  else
                  {
                     if(eval("\x01") == 430)
                     {
                        set("\x01",eval("\x01") + 99);
                        break loop0;
                     }
                     if(eval("\x01") != 529)
                     {
                        break;
                     }
                     set("\x01",eval("\x01") - 256);
                  }
               }
            }
         }
      }
   }
}
