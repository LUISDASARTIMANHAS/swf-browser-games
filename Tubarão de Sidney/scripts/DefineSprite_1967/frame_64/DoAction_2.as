function §\x04\x05§()
{
   set("\x03",645 % 511 * 5);
   return eval("\x03");
}
var §\x01§ = 176 + "\x04\x05"();
loop0:
while(true)
{
   if(eval("\x01") == 846)
   {
      set("\x01",eval("\x01") - 36);
      §§push(true);
      continue;
   }
   if(eval("\x01") == 349)
   {
      set("\x01",eval("\x01") + 415);
      §§push(!function §\x04\x05§()
      {
         set("\x03",645 % 511 * 5);
         return eval("\x03");
      });
      continue;
   }
   if(eval("\x01") == 810)
   {
      set("\x01",eval("\x01") - 740);
      if(function §\x04\x05§()
      {
         set("\x03",645 % 511 * 5);
         return eval("\x03");
      })
      {
         set("\x01",eval("\x01") + 239);
      }
      continue;
   }
   if(eval("\x01") == 70)
   {
      set("\x01",eval("\x01") + 239);
      break;
   }
   if(eval("\x01") == 309)
   {
      set("\x01",eval("\x01") + 651);
      continue;
   }
   if(eval("\x01") == 176)
   {
      set("\x01",eval("\x01") + 319);
      §§push(true);
      continue;
   }
   if(eval("\x01") == 162)
   {
      set("\x01",eval("\x01") + 14);
      continue;
   }
   if(eval("\x01") == 578)
   {
      set("\x01",eval("\x01") + 146);
      if(eval("{invalid_utf8:146}\x1b") >= 1)
      {
         §{invalid_utf8:146}\x1b§--;
         eval("{invalid_utf8:153}{invalid_utf8:145}{invalid_utf8:152}")["{invalid_utf8:229}{invalid_utf8:255}{invalid_utf8:230}{invalid_utf8:255}"]["\x14:"] -= eval(§§constant(4));
      }
      break;
   }
   if(eval("\x01") == 979)
   {
      set("\x01",eval("\x01") - 19);
      continue;
   }
   if(eval("\x01") == 598)
   {
      set("\x01",eval("\x01") + 289);
      continue;
   }
   if(eval("\x01") == 725)
   {
      set("\x01",eval("\x01") + 162);
      continue;
   }
   if(eval("\x01") == 495)
   {
      set("\x01",eval("\x01") + 386);
      if(function §\x04\x05§()
      {
         set("\x03",645 % 511 * 5);
         return eval("\x03");
      })
      {
         set("\x01",eval("\x01") - 156);
      }
      continue;
   }
   if(eval("\x01") != 881)
   {
      if(eval("\x01") == 960)
      {
         set("\x01",eval("\x01") - 409);
         §§push(true);
      }
      else if(eval("\x01") == 882)
      {
         set("\x01",eval("\x01") - 533);
         §§push(eval(function §\x04\x05§()
         {
            set("\x03",645 % 511 * 5);
            return eval("\x03");
         }));
      }
      else if(eval("\x01") == 764)
      {
         set("\x01",eval("\x01") - 610);
         if(function §\x04\x05§()
         {
            set("\x03",645 % 511 * 5);
            return eval("\x03");
         })
         {
            set("\x01",eval("\x01") + 424);
         }
      }
      else if(eval("\x01") == 438)
      {
         set("\x01",eval("\x01") + 92);
         var §§pop() = function §\x04\x05§()
         {
            set("\x03",645 % 511 * 5);
            return eval("\x03");
         };
      }
      else if(eval("\x01") == 175)
      {
         set("\x01",eval("\x01") + 1);
      }
      else if(eval("\x01") == 887)
      {
         set("\x01",eval("\x01") - 449);
         §§push("\x0f");
         §§push(1);
      }
      else if(eval("\x01") == 551)
      {
         set("\x01",eval("\x01") + 248);
         if(function §\x04\x05§()
         {
            set("\x03",645 % 511 * 5);
            return eval("\x03");
         })
         {
            set("\x01",eval("\x01") - 624);
         }
      }
      else
      {
         if(eval("\x01") == 799)
         {
            set("\x01",eval("\x01") - 624);
            break;
         }
         if(eval("\x01") == 530)
         {
            set("\x01",eval("\x01") + 352);
            §§push("\x0f");
         }
         else
         {
            if(eval("\x01") != 154)
            {
               if(eval("\x01") == 724)
               {
                  set("\x01",eval("\x01") - 724);
                  break;
               }
               break;
            }
            set("\x01",eval("\x01") + 424);
         }
      }
      continue;
   }
   set("\x01",eval("\x01") - 156);
   while(true)
   {
      if(function §\x04\x05§()
      {
         set("\x03",645 % 511 * 5);
         return eval("\x03");
      } == 796)
      {
         set("\x01",eval("\x01") - 316);
      }
      else if(eval("\x01") == 227)
      {
         set("\x01",eval("\x01") + 435);
         if(§§pop())
         {
            set("\x01",eval("\x01") - 298);
         }
      }
      else if(eval("\x01") == 80)
      {
         set("\x01",eval("\x01") + 147);
         §§push(!§§pop());
      }
      else if(eval("\x01") == 473)
      {
         set("\x01",eval("\x01") - 393);
         §§push(eval(§§pop()));
      }
      else if(eval("\x01") == 560)
      {
         set("\x01",eval("\x01") - 80);
      }
      else if(eval("\x01") == 735)
      {
         set("\x01",eval("\x01") + 114);
         if(§§pop())
         {
            set("\x01",eval("\x01") - 53);
         }
      }
      else
      {
         if(eval("\x01") == 849)
         {
            set("\x01",eval("\x01") - 53);
            break loop0;
         }
         if(eval("\x01") == 610)
         {
            set("\x01",eval("\x01") - 358);
            var §§pop() = §§pop();
         }
         else
         {
            if(eval("\x01") != 252)
            {
               if(eval("\x01") == 364)
               {
                  set("\x01",eval("\x01") + 412);
                  centerwurst._root.game += _x;
                  break loop0;
               }
               if(eval("\x01") == 776)
               {
                  set("\x01",eval("\x01") - 776);
                  break loop0;
               }
               break loop0;
            }
            set("\x01",eval("\x01") + 221);
            §§push("\x0f");
         }
      }
      while(true)
      {
         if(eval("\x01") == 112)
         {
            set("\x01",eval("\x01") + 623);
            §§push(true);
         }
         else if(eval("\x01") == 480)
         {
            set("\x01",eval("\x01") + 130);
            §§push("\x0f");
            §§push(1);
         }
         else
         {
            if(eval("\x01") != 662)
            {
               break;
            }
            set("\x01",eval("\x01") - 298);
         }
      }
   }
}
