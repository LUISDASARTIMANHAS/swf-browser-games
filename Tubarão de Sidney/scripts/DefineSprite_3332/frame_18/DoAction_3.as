function §\x04\x05§()
{
   set("\x03",1766 % 511 * 5);
   return eval("\x03");
}
var §\x01§ = -429 + "\x04\x05"();
loop0:
while(true)
{
   if(eval("\x01") == 736)
   {
      set("\x01",eval("\x01") - 346);
      §§push(true);
   }
   else
   {
      if(eval("\x01") == 473)
      {
         set("\x01",eval("\x01") + 49);
         loop1:
         while(true)
         {
            set("\x01",eval("\x01") - 127);
            §§push(eval(function §\x04\x05§()
            {
               set("\x03",1766 % 511 * 5);
               return eval("\x03");
            }));
            while(true)
            {
               if(eval("\x01") == 6)
               {
                  set("\x01",eval("\x01") + 94);
                  §§push(true);
               }
               else if(eval("\x01") == 603)
               {
                  set("\x01",eval("\x01") - 230);
                  §§push(!§§pop());
               }
               else if(eval("\x01") == 100)
               {
                  set("\x01",eval("\x01") + 436);
                  if(§§pop())
                  {
                     set("\x01",eval("\x01") + 126);
                  }
               }
               else
               {
                  if(eval("\x01") == 859)
                  {
                     set("\x01",eval("\x01") - 195);
                     eval("{invalid_utf8:193}{invalid_utf8:242}{invalid_utf8:229}\t")["{invalid_utf8:218}{invalid_utf8:216}U"]["{invalid_utf8:226}y{invalid_utf8:228}\x03"] += eval(§§constant(3)) * eval("{invalid_utf8:193}{invalid_utf8:242}{invalid_utf8:229}\t")[§§constant(4)][§§constant(5)];
                     break loop0;
                  }
                  if(eval("\x01") == 662)
                  {
                     set("\x01",eval("\x01") + 56);
                  }
                  else if(eval("\x01") == 895)
                  {
                     set("\x01",eval("\x01") - 177);
                  }
                  else if(eval("\x01") == 363)
                  {
                     set("\x01",eval("\x01") + 366);
                     §§push("\x0f");
                     §§push(1);
                  }
                  else if(eval("\x01") == 676)
                  {
                     set("\x01",eval("\x01") - 313);
                  }
                  else if(eval("\x01") == 718)
                  {
                     set("\x01",eval("\x01") - 197);
                     §§push(true);
                  }
                  else
                  {
                     if(eval("\x01") == 246)
                     {
                        break loop1;
                     }
                     if(eval("\x01") == 521)
                     {
                        set("\x01",eval("\x01") - 275);
                        if(§§pop())
                        {
                           set("\x01",eval("\x01") - 169);
                        }
                     }
                     else
                     {
                        if(eval("\x01") == 536)
                        {
                           set("\x01",eval("\x01") + 126);
                           break loop0;
                        }
                        if(eval("\x01") == 374)
                        {
                           set("\x01",eval("\x01") + 485);
                        }
                        else
                        {
                           if(eval("\x01") == 730)
                           {
                              break;
                           }
                           if(eval("\x01") == 77)
                           {
                              set("\x01",eval("\x01") + 286);
                           }
                           else if(eval("\x01") == 373)
                           {
                              set("\x01",eval("\x01") + 1);
                              if(§§pop())
                              {
                                 set("\x01",eval("\x01") + 485);
                              }
                           }
                           else if(eval("\x01") == 942)
                           {
                              set("\x01",eval("\x01") - 212);
                              §§push("\x0f");
                           }
                           else
                           {
                              if(eval("\x01") != 729)
                              {
                                 if(eval("\x01") == 664)
                                 {
                                    set("\x01",eval("\x01") - 664);
                                    break loop0;
                                 }
                                 break loop0;
                              }
                              set("\x01",eval("\x01") + 213);
                              var §§pop() = §§pop();
                           }
                        }
                     }
                  }
               }
            }
         }
         set("\x01",eval("\x01") - 169);
         §§enumerate(getTimer());
         §§push(§§pop() == ord(new §function \§\\x04\\x05\§()\r\n{\r\n   set("\\x03",1766 % 511 * 5);\r\n   return eval("\\x03");\r\n}§()));
         break;
      }
      if(eval("\x01") == 522)
      {
         set("\x01",eval("\x01") - 200);
      }
      else if(eval("\x01") == 390)
      {
         set("\x01",eval("\x01") + 83);
         if(function §\x04\x05§()
         {
            set("\x03",1766 % 511 * 5);
            return eval("\x03");
         })
         {
            set("\x01",eval("\x01") + 49);
         }
      }
      else if(eval("\x01") == 82)
      {
         set("\x01",eval("\x01") + 240);
      }
      else
      {
         if(eval("\x01") == 582)
         {
            set("\x01",eval("\x01") - 195);
            _root.board.score += z * _root.kombi.combo;
            break;
         }
         if(eval("\x01") == 616)
         {
            set("\x01",eval("\x01") - 34);
         }
         else if(eval("\x01") == 809)
         {
            set("\x01",eval("\x01") - 290);
            §§push(eval(function §\x04\x05§()
            {
               set("\x03",1766 % 511 * 5);
               return eval("\x03");
            }));
         }
         else if(eval("\x01") == 46)
         {
            set("\x01",eval("\x01") + 570);
            if(function §\x04\x05§()
            {
               set("\x03",1766 % 511 * 5);
               return eval("\x03");
            })
            {
               set("\x01",eval("\x01") - 34);
            }
         }
         else if(eval("\x01") == 322)
         {
            set("\x01",eval("\x01") + 105);
            §§push("\x0f");
            §§push(1);
         }
         else if(eval("\x01") == 427)
         {
            set("\x01",eval("\x01") - 319);
            var §§pop() = function §\x04\x05§()
            {
               set("\x03",1766 % 511 * 5);
               return eval("\x03");
            };
         }
         else if(eval("\x01") == 108)
         {
            set("\x01",eval("\x01") + 701);
            §§push("\x0f");
         }
         else
         {
            if(eval("\x01") != 519)
            {
               if(eval("\x01") == 387)
               {
                  set("\x01",eval("\x01") - 387);
                  break;
               }
               break;
            }
            set("\x01",eval("\x01") - 473);
            §§push(!function §\x04\x05§()
            {
               set("\x03",1766 % 511 * 5);
               return eval("\x03");
            });
         }
      }
   }
}
