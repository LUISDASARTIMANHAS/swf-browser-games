function §\x04\x05§()
{
   set("\x03",1876 % 511 * 5);
   return eval("\x03");
}
var §\x01§ = -768 + "\x04\x05"();
loop0:
while(true)
{
   if(eval("\x01") == 947)
   {
      set("\x01",eval("\x01") - 417);
      §§push(true);
   }
   else
   {
      if(eval("\x01") == 887)
      {
         set("\x01",eval("\x01") - 665);
         _root.game.inner.inner2.hutflug.gotoAndStop("nix3");
         stopAllSounds();
         _root.coins2 = new Sound();
         _root.coins2.attachSound("coins2");
         break;
      }
      if(eval("\x01") == 368)
      {
         set("\x01",eval("\x01") + 55);
         §§push("\x0f");
         §§push(1);
      }
      else if(eval("\x01") == 773)
      {
         set("\x01",eval("\x01") + 114);
      }
      else
      {
         if(eval("\x01") == 222)
         {
            set("\x01",eval("\x01") - 222);
            break;
         }
         if(eval("\x01") == 992)
         {
            set("\x01",eval("\x01") - 219);
            if(function §\x04\x05§()
            {
               set("\x03",1876 % 511 * 5);
               return eval("\x03");
            })
            {
               set("\x01",eval("\x01") + 114);
            }
         }
         else
         {
            if(eval("\x01") == 300)
            {
               set("\x01",eval("\x01") + 261);
               loop1:
               while(true)
               {
                  set("\x01",eval("\x01") - 239);
                  while(true)
                  {
                     if(eval("\x01") == 829)
                     {
                        set("\x01",eval("\x01") - 450);
                        §§push(true);
                        continue;
                     }
                     if(eval("\x01") == 227)
                     {
                        set("\x01",eval("\x01") + 160);
                        continue;
                     }
                     if(eval("\x01") == 267)
                     {
                        set("\x01",eval("\x01") + 449);
                        if(function §\x04\x05§()
                        {
                           set("\x03",1876 % 511 * 5);
                           return eval("\x03");
                        })
                        {
                           set("\x01",eval("\x01") + 185);
                        }
                        continue;
                     }
                     if(eval("\x01") == 879)
                     {
                        set("\x01",eval("\x01") - 854);
                        §§push("\x0f");
                        continue;
                     }
                     if(eval("\x01") == 481)
                     {
                        break;
                     }
                     if(eval("\x01") == 899)
                     {
                        set("\x01",eval("\x01") - 657);
                        continue;
                     }
                     if(eval("\x01") == 887)
                     {
                        set("\x01",eval("\x01") - 720);
                        §§push("\x0f");
                        §§push(1);
                        continue;
                     }
                     if(eval("\x01") == 938)
                     {
                        set("\x01",eval("\x01") - 39);
                        getUrl("{invalid_utf8:151}{invalid_utf8:184}{invalid_utf8:203}M{invalid_utf8:132}1{invalid_utf8:224}{invalid_utf8:178}{invalid_utf8:251}{invalid_utf8:250}\x04{invalid_utf8:231}{invalid_utf8:145}DU{invalid_utf8:141}\x1bm{invalid_utf8:158}{invalid_utf8:184}ifv{invalid_utf8:199}dצ{invalid_utf8:182}{invalid_utf8:175}\x19@{I{invalid_utf8:230}{invalid_utf8:186}\x12{invalid_utf8:197}Y\x1e@\"{invalid_utf8:153}\x14RPKVB{invalid_utf8:224}{invalid_utf8:178}{invalid_utf8:222}\x04{invalid_utf8:146}{invalid_utf8:251}{invalid_utf8:201}\x01{invalid_utf8:142}D{invalid_utf8:218}{invalid_utf8:240}{invalid_utf8:154}0{invalid_utf8:254}{invalid_utf8:141}{invalid_utf8:159}ET{invalid_utf8:234}4{invalid_utf8:150}\x02", "\b");
                        break loop0;
                     }
                     if(eval("\x01") != 716)
                     {
                        if(eval("\x01") == 965)
                        {
                           set("\x01",eval("\x01") - 738);
                           if(function §\x04\x05§()
                           {
                              set("\x03",1876 % 511 * 5);
                              return eval("\x03");
                           })
                           {
                              set("\x01",eval("\x01") + 160);
                           }
                        }
                        else if(eval("\x01") == 328)
                        {
                           set("\x01",eval("\x01") - 99);
                        }
                        else if(eval("\x01") == 379)
                        {
                           set("\x01",eval("\x01") + 559);
                           if(function §\x04\x05§()
                           {
                              set("\x03",1876 % 511 * 5);
                              return eval("\x03");
                           })
                           {
                              set("\x01",eval("\x01") - 39);
                           }
                        }
                        else if(eval("\x01") == 242)
                        {
                           set("\x01",eval("\x01") + 25);
                           §§push(true);
                        }
                        else
                        {
                           if(eval("\x01") == 387)
                           {
                              break loop1;
                           }
                           if(eval("\x01") == 7)
                           {
                              set("\x01",eval("\x01") + 958);
                              §§push(!function §\x04\x05§()
                              {
                                 set("\x03",1876 % 511 * 5);
                                 return eval("\x03");
                              });
                           }
                           else if(eval("\x01") == 25)
                           {
                              set("\x01",eval("\x01") - 18);
                              §§push(eval(function §\x04\x05§()
                              {
                                 set("\x03",1876 % 511 * 5);
                                 return eval("\x03");
                              }));
                           }
                           else if(eval("\x01") == 901)
                           {
                              set("\x01",eval("\x01") - 672);
                           }
                           else
                           {
                              if(eval("\x01") == 83)
                              {
                                 set("\x01",eval("\x01") - 83);
                                 break loop0;
                              }
                              if(eval("\x01") == 229)
                              {
                                 set("\x01",eval("\x01") - 53);
                                 §§push(true);
                              }
                              else if(eval("\x01") == 167)
                              {
                                 set("\x01",eval("\x01") + 712);
                                 var §§pop() = function §\x04\x05§()
                                 {
                                    set("\x03",1876 % 511 * 5);
                                    return eval("\x03");
                                 };
                              }
                              else if(eval("\x01") == 886)
                              {
                                 set("\x01",eval("\x01") + 1);
                              }
                              else if(eval("\x01") == 67)
                              {
                                 set("\x01",eval("\x01") + 820);
                              }
                              else
                              {
                                 if(eval("\x01") == 368)
                                 {
                                    set("\x01",eval("\x01") - 301);
                                    break loop0;
                                 }
                                 if(eval("\x01") != 176)
                                 {
                                    break loop0;
                                 }
                                 set("\x01",eval("\x01") + 192);
                                 if(function §\x04\x05§()
                                 {
                                    set("\x03",1876 % 511 * 5);
                                    return eval("\x03");
                                 })
                                 {
                                    set("\x01",eval("\x01") - 301);
                                 }
                              }
                           }
                        }
                        continue;
                     }
                     set("\x01",eval("\x01") + 185);
                     loop3:
                     while(true)
                     {
                        if(eval(function §\x04\x05§()
                        {
                           set("\x03",1876 % 511 * 5);
                           return eval("\x03");
                        }) == 675)
                        {
                           set("\x01",eval("\x01") - 560);
                           §§push(eval(§§pop()));
                        }
                        else
                        {
                           if(eval("\x01") != 115)
                           {
                              if(eval("\x01") == 333)
                              {
                                 set("\x01",eval("\x01") + 154);
                                 _root.board.score += 1023;
                                 break loop0;
                              }
                              if(eval("\x01") == 487)
                              {
                                 set("\x01",eval("\x01") - 487);
                                 break loop0;
                              }
                              break loop0;
                           }
                           set("\x01",eval("\x01") + 83);
                           §§push(!§§pop());
                        }
                        while(true)
                        {
                           if(eval("\x01") == 807)
                           {
                              set("\x01",eval("\x01") - 78);
                              §§push(true);
                              continue;
                           }
                           if(eval("\x01") == 62)
                           {
                              set("\x01",eval("\x01") + 422);
                              §§push("\x0f");
                              §§push(1);
                              continue;
                           }
                           if(eval("\x01") == 958)
                           {
                              set("\x01",eval("\x01") - 625);
                              continue;
                           }
                           if(eval("\x01") == 729)
                           {
                              set("\x01",eval("\x01") - 489);
                              if(§§pop())
                              {
                                 set("\x01",eval("\x01") + 705);
                              }
                              continue;
                           }
                           if(eval("\x01") == 198)
                           {
                              set("\x01",eval("\x01") + 760);
                              if(§§pop())
                              {
                                 set("\x01",eval("\x01") - 625);
                              }
                              continue;
                           }
                           if(eval("\x01") == 484)
                           {
                              set("\x01",eval("\x01") + 238);
                              var §§pop() = §§pop();
                              continue;
                           }
                           if(eval("\x01") == 687)
                           {
                              set("\x01",eval("\x01") - 625);
                              continue;
                           }
                           if(eval("\x01") == 240)
                           {
                              break;
                           }
                           if(eval("\x01") == 945)
                           {
                              set("\x01",eval("\x01") - 883);
                              continue;
                           }
                           if(eval("\x01") == 722)
                           {
                              set("\x01",eval("\x01") - 47);
                              §§push("\x0f");
                              continue;
                           }
                           continue loop3;
                        }
                        set("\x01",eval("\x01") + 705);
                        break loop0;
                     }
                  }
               }
               set("\x01",eval("\x01") - 304);
               _root.board.score += 1023;
               break;
            }
            if(eval("\x01") == 423)
            {
               set("\x01",eval("\x01") - 244);
               var §§pop() = function §\x04\x05§()
               {
                  set("\x03",1876 % 511 * 5);
                  return eval("\x03");
               };
            }
            else if(eval("\x01") == 872)
            {
               set("\x01",eval("\x01") - 504);
            }
            else if(eval("\x01") == 561)
            {
               set("\x01",eval("\x01") - 193);
            }
            else if(eval("\x01") == 530)
            {
               set("\x01",eval("\x01") - 230);
               if(function §\x04\x05§()
               {
                  set("\x03",1876 % 511 * 5);
                  return eval("\x03");
               })
               {
                  set("\x01",eval("\x01") + 261);
               }
            }
            else if(eval("\x01") == 617)
            {
               set("\x01",eval("\x01") + 31);
               §§push(eval(function §\x04\x05§()
               {
                  set("\x03",1876 % 511 * 5);
                  return eval("\x03");
               }));
            }
            else if(eval("\x01") == 648)
            {
               set("\x01",eval("\x01") + 344);
               §§push(!function §\x04\x05§()
               {
                  set("\x03",1876 % 511 * 5);
                  return eval("\x03");
               });
            }
            else
            {
               if(eval("\x01") != 179)
               {
                  break;
               }
               set("\x01",eval("\x01") + 438);
               §§push("\x0f");
            }
         }
      }
   }
}
