package
{
   ...
   [Embed(source="/_assets/assets.swf", symbol="CrewSecuritylv1")]
               ...
               while(i < myParent.banditList.length)
               {
                  ...
                  if(temp.floorPos == floorPos && !temp.rideElevator && !rideElevator && temp.pursuer.length < 3 && !temp.captured && temp.alpha == 1)
                  {
                     if(found == null)
                     {
                        ...
                        {
                           if(temp == myParent.banditTrigger || Math.abs(temp.worldX - worldX) <= 300 && Math.abs(temp.worldX - worldX) < Math.abs(found.worldX - worldX))
                           {
                              found = temp;
                           }
                        }
                        else if(Math.abs(temp.worldX - worldX) <= 200)
                        {
                           if(dirrection > 0)
                           {
